...
ProjectScope.tsx                    SHARED "This project / All" scope toggle + `scopeByProject` helper.
                                    Follows the active project on tenant-wide registers (TAC, Controls,
                                    Incidents, Escalations, Resource Planner). `includeUntagged` opt
                                    (default true = org-wide records show in project view; RP passes
                                    false = unassigned = portfolio-only). See the Project-scope convention.
StatsCard.tsx                       KPI tile (title / value / icon) used across dashboards + monitoring.
```
...
routes/data.ts                     Generic save/getData + evidence CRUD. `addEvidence` accepts
                                   ...
                                   `saveData`'s whole-array project branch (risks/issues/kris/
                                   complianceItems/tasks + history row + logArrayChanges + severe-risk
                                   escalation) is EXTRACTED to `api/lib/legacyArrayWrite.ts`
                                   (`writeLegacyArray`); saveData keeps its authz + response and
                                   delegates, so the agentic layer's apply-adapters share one write
                                   path and the side-effects can't drift.
...
### Compliance-item stage vocabulary (load-bearing — filters break silently against the wrong value)
A compliance item's lifecycle is its `stage` field. The **canonical set** is defined by `COMPLIANCE_STAGES` in [`web/features/compliance/pages/ComplianceTracker.tsx`](web/features/compliance/pages/ComplianceTracker.tsx): `'Information Gap' | 'Risk Identified' | 'In Progress' | 'Live' | 'Archived'`. Derived facts every consumer must use:
- **"Completed" = `stage === 'Live' || stage === 'Archived'`.** There is **no `'Complete'` stage** — filtering on `'Complete'` matches nothing (this silently made the Executive report's compliance KPI always 0%). Likewise there is **no `'At Risk'` stage** — "at risk / critical" = `'Risk Identified' || 'Information Gap'` (cf. `ComplianceTracker` `critical`/`completed` derivations).
- **"Not started" has two real values:** `ComplianceSetup` stamps newly-created items `stage: 'Not Started'` ([`ComplianceSetup.tsx`](web/features/compliance/pages/ComplianceSetup.tsx)), while seed/demo items default to `'Information Gap'`. A "not started" filter should accept **both** (`'Not Started'` is NOT in `COMPLIANCE_STAGES` but is a real persisted value from setup — do not "clean it up" away).
- A "not done / pending" predicate is `stage !== 'Live' && stage !== 'Archived'` — never `!== 'Complete'`.
- `completedAt` is stamped only when an item reaches `Live`/`Archived` (drives `ComplianceVelocityChart`). The `dueDate` field exists on items and is the basis for overdue alerts.
...
surfaced under the **"Escalations & Incidents" sidebar group** (renamed 2026-07 from "Assurance" — label + the 4 inner breadcrumbs only; route `/assurance`, collection `assuranceAlerts`, and all `assurance*`/`ASSURANCE_*` code names UNCHANGED) (`hasCoreAccess`-gated). Load-bearing facts:
...
### Automatic detection & alert engine conventions (2026-07 — Q2 build)
The cross-platform alert engine detects problem signals server-side and delivers them **in-app + FCM push**. It generalises the governance chase-engine pattern — do NOT re-implement detection inside React render code.
- **Pure detectors live in [`api/lib/alertEngine/detectors.ts`](api/lib/alertEngine/detectors.ts)** (`computeDetectedAlerts(input, thresholds, now)` → `DetectedAlert[]`), server-owned, unit-tested (`api/__tests__/alertDetectors.test.ts`), NO web imports (respects the api→web boundary — the recurrence rule is re-implemented, not imported from `web/lib/learning/recurrence.ts`). Six state-based signals: evidence-missing, compliance-overdue, capa-overdue, incident-stale (7-day), incident-recurring (90d/2×), risk-overdue. The 7th — **risk turns Severe — is event-driven** in [`api/lib/riskSevereEscalation.ts`](api/lib/riskSevereEscalation.ts) `writeSevereEscalations` (which now ACTUALLY dispatches FCM + emits a `risk-severe` `detectedAlerts` row — the old detect-but-never-send gap is fixed). "Board/regulator reporting" stays the existing governance chase engine (Q2.5).
- **Thresholds** via [`api/lib/alertConfig.ts`](api/lib/alertConfig.ts) — per-workspace `alertConfig/{clientId}` doc merged over locked defaults (immediate overdue, 7-day stale, 90d/2× recurrence). Absent doc ⇒ defaults.
- **The cron handler is [`api/routes/alertEngine.ts`](api/routes/alertEngine.ts) `runAlertEngine`** (modelled on `governanceCron.ts`): hourly (`vercel.json` `:30`), `CRON_SECRET` auth via `isAuthorisedCronCall`, multi-tenant workspace sweep, reads each workspace's project+programme `projects/{id}/data/{complianceItems,risks,tasks}` + tenant `incidents`, runs detectors, **24h dedupe** (`${signalKind}:${entityId}` key) on the **`detectedAlerts`** collection, persists a row, FCM-pushes to **default recipients = workspace owner + client_admins** (until the client routing table lands), 90-day TTL purge. Needs the `detectedAlerts` composite index in [`firestore.indexes.json`](firestore.indexes.json) (clientId+dedupeKey+createdAt).
- **NEVER auto-escalate detected signals into `assuranceAlerts`** — that stays a human action (it fires per-alert LLM cost). NEVER wire email (in-app + push only). Per-signal opt-out via `users.notificationPreferences.alerts` / `.alertSignals[kind]`.
- **In-app surface:** store slice `detectedAlerts` + `loadDetectedAlerts` / `markDetectedAlertRead` (optimistic + rollback); client reads via `api.listDetectedAlerts` / `api.markDetectedAlertRead`. Rendered on the **Detected Alerts** page ([`web/features/reporting/pages/AlertsPage.tsx`](web/features/reporting/pages/AlertsPage.tsx), `/monitoring/detected-alerts`, PM+); FCM foreground pushes also feed the existing notification bell. The **Monitoring Overview** page ([`web/features/reporting/pages/MonitoringOverviewPage.tsx`](web/features/reporting/pages/MonitoringOverviewPage.tsx), `/monitoring/overview`, PM+) assembles the existing `AssuranceSignalsCard` + `DashboardGovernanceCard` + `ComplianceVelocityChart` + a resource summary. The **Monitoring & Reporting sidebar group is now `hasCoreAccess` (PM+)**, not Client-Admin-only.

### Project-scope filter convention (2026-07 — Q5 hybrid)
Tenant-wide registers get a **"This project / All" toggle that follows the active project** via the shared [`web/components/common/ProjectScope.tsx`](web/components/common/ProjectScope.tsx) — `ProjectScopeToggle` + `scopeByProject(items, scope, activeProjectId, { includeUntagged })`. Pattern (copy `IncidentsRegisterPage`): `scope` state seeded from `activeProjectId`, a `useEffect` re-syncing on `activeProjectId` change, a `useMemo` applying `scopeByProject`, and the toggle in the `PageHeader` actions (only when a project is active). **`includeUntagged` defaults true** (org-wide/untagged records show in every project's view — Controls/Incidents/Escalations); **Resource Planner passes `includeUntagged: false`** (an unassigned scheme is portfolio-only). Applied to: TAC enquiries, Controls/Incidents/Escalations registers, and all 5 Resource Planner pages. Governance reports/meetings were deliberately left council-wide (no project field); RP schemes gained a real `projectId` FK (server already persisted arbitrary scheme fields — no API/migration change).

### Controls ↔ AI mitigation convention (2026-07 — Q3)
The AI **Mitigation & Control Strategy** page ([`AIControlSuggestions.tsx`](web/features/compliance/pages/AIControlSuggestions.tsx)) can **"Promote to Controls register"** — mints a first-class `Control` via `saveControl` with `origin: 'ai-suggestion'` + `sourceRiskId` (both whitelisted in `sanitizeControl`, [`api/routes/controls.ts`](api/routes/controls.ts)), server-assigned id, deduped by source-risk+title. The original "add to risk notes" button is kept alongside it. `RiskModal` shows a read-only "Linked controls · register" list (Controls whose `linkedRiskIds` include the risk). `canManageControls` (store) now matches the server gate (PM+ **and** Programme Manager). New-feature prose fields (incident root cause/lessons, control description, escalation description, TAC enquiry/RFI body) gained the `<AIWriter>` helper; meeting minutes deliberately excluded (rich editor has its own AIDraftModal). Regulation library items gained an optional `sourceUrl` rendered as a clickable "Official Source" link.

### Agentic Layer conventions (2026-07 — 7 on-demand AI agents)
A controlled, auditable, source-backed **Human + AI layer on top of** the existing modules (NOT a rewrite of them). Officers run an agent → it reads tenant-scoped records → drafts cited SUGGESTIONS → a human reviews (accept / edit / reject / regenerate) → after approval a server writer applies them into live records. **Nothing auto-applies.**

- **7 agents** (`AgentKey`): `riskIncident`, `compliance`, `governance`, `evidence`, `technical` (web-grounded, takes a question), `monitoring`, `delivery`. Each is a registry entry in [`api/lib/agents/defs/`](api/lib/agents/defs/) (`retrieve()` + `buildPrompt()`), registered in `defs/index.ts` (a side-effect barrel imported once by the route). **To add an agent:** copy a def, reuse the retrieval helpers, add an adapter only for a genuinely new output type. Agents add **NO new API action** (they run via `agentRun`).
- **Single-run generators, NOT conversational** (client-locked). Retrieval is **code-directed** ([`api/lib/agents/retrieval.ts`](api/lib/agents/retrieval.ts) `readContextArray`/`readTenantCollection`/`readContextEvidence`/`BundleBuilder`) — the model NEVER chooses what to fetch, so tenant isolation + permission inheritance are structural. Runs execute AS the requesting user; `resolveScope` calls `ctx.isAuthorizedForContext` before any read.
- **Pipeline** [`api/lib/agents/pipeline.ts`](api/lib/agents/pipeline.ts) `runAgentPipeline`: authorize → retrieve → fence → model → validate → persist. The run doc is written BEFORE the model call so a failed/timed-out run stays auditable. `validateSuggestion` is the guardrail validator: drops disallowed output types, **drops claimed `sourceIds` not in the retrieved set** (hallucinated citation → gone), flags a zero-source suggestion unsupported + caps confidence. All AI via `runAIOperation` (**never** `api/routes/ai.ts`); the Technical Companion uses the two-call web-grounded pattern (`webSearch` ⊕ `responseSchema`), web text fenced as untrusted.
- **Injection fencing** [`api/lib/agents/fencing.ts`](api/lib/agents/fencing.ts): `clean` (strips `<>`/newlines, single-line fields) + `cleanBlock` (keeps newlines, web/question) + `fenceRecords` (`<TAG>[id] label | fields`) + `FENCE_PREAMBLE`. Tenant record text and the user question are DATA, never instructions.
- **Two collections** (`shared/types/agents.ts` = the data contract, brief §5): `agentSuggestions` (one doc per suggestion: payload / editedPayload / citations{records,documents,web} / confidence / assumptions / missingEvidence / reviewStatus / reviewer / applied) + `agentRuns` (prompt-context audit). **Queries are single-equality `where clientId ==` + in-memory filter/sort — NO composite index** (never edit `firestore.indexes.json` for this).
- **Approval-before-write is SERVER-enforced.** `agentApplySuggestion` ([`api/routes/agents.ts`](api/routes/agents.ts)) is the ONLY action that writes a live record: PM+ (`canReview` = `isClientAdmin || canReviewAgentSuggestions(role)`), only an `accepted`/`edited` suggestion, re-checks tenant + context authz + sanitize + prohibited at apply time, and **claims the suggestion transactionally (approved → applied) BEFORE writing** so a concurrent double-apply or an apply-racing-a-reject can't write twice or write a rejected item; on write failure the claim is released. `agentReviewSuggestion` (PM+) does accept/edit(keeps original + diff)/reject(reason required). `agentRegenerate` re-runs + supersedes THIS agent+context's prior DRAFTS only.
- **Prohibited actions are UNREPRESENTABLE, not merely blocked** ([`api/lib/agents/adapters/`](api/lib/agents/adapters/)): there is **no adapter** that closes an incident, downgrades a risk, marks compliance complete/verified, resolves an escalation, sends external comms or commits spend. `base.ts` strips a `FORBIDDEN_FIELDS` set (reviewedBy/approvedBy/closedAt/completedAt/verifiedBy…); the incident adapter merges only descriptive fields + strips status/closedAt + authorizes the TARGET incident's project; controls force `Not Tested`; compliance forces `Information Gap`; escalation forces `Open`; `narrative`/`technicalAnswer` are **accept-terminal** (no adapter). Array-backed writes go through [`api/lib/legacyArrayWrite.ts`](api/lib/legacyArrayWrite.ts) (extracted from `saveData` — human + agent writes share ONE path so history/audit/severe-escalation can't drift), idempotent by `aiSuggestionId`.
- **Visibility:** suggestions are tenant-visible to context-authorized users EXCEPT `technicalAnswer` (inherits TAC owner-scoping via exported `isTacElevated`); the list + run-audit filter by `isAuthorizedForContext` (no cross-project leak); `agentListRuns` is PM+ (holds the prompt dump). **`viewer` is denied by default** — agent actions are OFF the `viewerGate` allowlist → ACCESS_RESTRICTED → Request-Access modal → admin upgrades the role. **Never edit `api/lib/viewerGate.ts` for this.**
- **UI** [`web/features/agents/`](web/features/agents/): `/agents/suggestions` review queue (`AgentSuggestionsPage`, DynamicTable, "AI Agents" sidebar group gated `hasCoreAccess`, `ProjectScopeToggle` follows the active context) + `SuggestionReviewPanel` (sources / model-reported confidence / assumptions / missing evidence / accept-edit-reject-apply, PM+) + `RunAgentButton` (standard secondary-button style; Technical opens a question modal) driven by `useRunAgent` (core-access gate, resolves active scope, toast + deep-link). Store slice: pessimistic `agentSuggestions`/`loadAgentSuggestions`/`runAgent`(→ regenerate)/`reviewAgentSuggestion`/`applyAgentSuggestion`/`canReviewAgentSuggestions`. Model quality is an admin dial (operationModels / per-agent `modelOverride`), not code. Design doc: `.claude/plans/architecture-agentic-layer.md`.

### Standing rules for sweeps / refactors
- **The Agentic Layer goes through [`api/lib/agents/`](api/lib/agents/) (pipeline/registry/fencing/retrieval/adapters) + [`api/routes/agents.ts`](api/routes/agents.ts) + [`web/features/agents/`](web/features/agents/).** Agents are single-run + code-directed retrieval (never model-directed); `agentApplySuggestion` is the ONLY live-record writer (PM+, claim-first, accepted/edited only); prohibited actions are unrepresentable (no adapter — never add one that closes/downgrades/verifies/resolves/comms/spends); array writes via `writeLegacyArray`; all AI via `runAIOperation`; every new agent action needs a `shared/lib/apiCatalog.ts` entry; NEVER edit `firestore.indexes.json` or `viewerGate.ts` for it. See the Agentic Layer convention above.
- **The automatic alert engine goes through [`api/lib/alertEngine/detectors.ts`](api/lib/alertEngine/detectors.ts) (pure) + [`api/routes/alertEngine.ts`](api/routes/alertEngine.ts) (cron) + [`api/lib/alertConfig.ts`](api/lib/alertConfig.ts) (thresholds).** Detect server-side, never in React render code; write to `detectedAlerts` (NOT `assuranceAlerts` — no auto-escalation, no email); dedupe on `${signalKind}:${entityId}`; deliver in-app + FCM push with per-signal opt-out. See the alert engine convention above.
- **Project-scope filtering goes through [`web/components/common/ProjectScope.tsx`](web/components/common/ProjectScope.tsx) `scopeByProject` + `ProjectScopeToggle`** (`includeUntagged: false` for Resource Planner schemes). Don't re-derive the active-project filter inline.
