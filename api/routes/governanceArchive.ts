...
//   • reports with status === 'Sealed'
//   • meetings with status === 'Held' (/b)
//   • projectGovernanceDocs with status === 'Published'
//
// Endpoints:
...
async function governanceListArchive(req: any, res: any, ctx: ApiContext) {
  try {
    //  when `asOfMonth` (YYYY-MM) is in the request body, the
...
// honours Part 2 redaction ( watermark + planned
...
async function governanceExportArchiveFoi(_req: any, res: any, ctx: ApiContext) {
  try {
    ...
    // Header carries 4 trailing FP-item columns (people fields + approval
    // state). The current archive only emits sealed reports, held meetings,
    // and published docs, so these columns are blank today; they populate
    // once FP items become an archive kind. Consumers parsing by header
    // name remain compatible because the columns are appended at the end.
    const header = [
      'Kind',
      'Reference',
      'Title',
