...
export const API_ACTIONS: ApiActionDoc[] = [
  ...
  { action: "agentRegenerate", group: "AI Agents", title: "Regenerate an agent's suggestions", description: "Re-runs an agent over the same scope and supersedes the previous DRAFT suggestions for that agent and context, so re-running replaces stale drafts instead of duplicating them. Accepted, edited, rejected and applied suggestions are never superseded.", requiredRole: "Any signed-in user (workspace-scoped; viewers are denied)", params: [ { name: "agentKey", type: "string", required: true, description: "Which agent to re-run." }, { name: "contextKind", type: "string", required: true, description: "project | programme | portfolio." }, { name: "contextId", type: "string", description: "Project or programme id; required unless contextKind is portfolio." }, { name: "question", type: "string", description: "Free-text question — Technical Companion only." } ] },
