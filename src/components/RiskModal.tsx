...
import { WORKSTREAMS, CATEGORIES, KRI_LIST, RISK_STATUSES, RISK_RESPONSES, APPETITES } from '../data/riskData';
import { X, ShieldAlert, Target, AlertTriangle, ArrowRight } from 'lucide-react';
...
interface RiskModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSave: (risk: Partial<RiskItem>) => void;
...
export function RiskModal({ isOpen, onClose, onSave, initialData }: RiskModalProps) {
    const { projects, programmes, activeProjectId, activeProgrammeId, tasks, addTask, updateTask, deleteTask } = useStore();
    ...
    }, [initialData, isOpen, activeProjectId, activeProgrammeId, projects, programmes]);
    ...
    const handleChange = (field: keyof RiskItem, value: any) => {
        setFormData(prev => {
            ...
            if (field === 'residualL') {
                next.residualProb = L_TO_PCT[parseInt(value) as 1|2|3|4|5] ?? 20;
            }
    ...
    const handleSave = () => {
        ...
        updated.residualRating = (updated.residualL || 1) * (updated.residualI || 1);
        updated.grossALE = (updated.grossImpact || 0) * gProb;
        updated.residualALE = (updated.residualImpact || 0) * rProb;
        updated.riskReduction = (updated.grossALE || 0) - (updated.residualALE || 0);
        updated.riskReductionPct = (updated.grossRating && updated.grossRating > 0)
            ? Math.round((1 - (updated.residualRating || 0) / updated.grossRating) * 100)
            : 0;
    ...
    const scoreColor = (score: number) => {
        if (!score || score <= 6) return "bg-emerald-100 text-emerald-800 border-emerald-200";
    ...
    const rALE = (formData.residualImpact || 0) * ((formData.residualProb || 0) / 100);

    // Projects scoped to the selected/active programme
    const safeProjects = Array.isArray(projects) ? projects : [];
    const scopedProjects = activeProgrammeId
        ? safeProjects.filter(p => p.programmeId === (formData.programmeId || activeProgrammeId))
        : safeProjects;
            ...
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl max-h-[calc(100dvh-2rem)] md:max-h-[90vh] flex flex-col">
                ...
                <div className="flex items-center justify-between p-6 border-b border-slate-100 shrink-0">
                    <div>
                        ...
                        <h2 className="text-xl font-black text-slate-900 tracking-tight uppercase italic">{initialData ? 'Refine Risk Intelligence' : 'Register New Risk'}</h2>
                        {formData.escalated && (
                            <div className="flex items-center gap-2 px-3 py-1 bg-purple-600 text-white rounded-full animate-pulse shadow-lg shadow-purple-200">
                                ...
                                <span className="text-[10px] font-black uppercase tracking-widest">Escalated to Programme</span>
                            </div>
                        )}
                    </div>
                ...
                <div className="shrink-0 px-6 pt-2 space-y-2">
                    {formData.escalated && (
                        <div className="flex items-center gap-3 p-3 bg-amber-50 border border-amber-200 rounded-xl text-amber-800">
                            ...
                            <div className="text-xs font-bold leading-tight">
                                <span className="uppercase tracking-widest block mb-0.5">Escalated to Programme</span>
                                This risk is currently being managed at the programme level. Any changes here will be reflected in the Programme Risk Register.
                            </div>
                        </div>
                    )}
                    {formData.convertedToIssue && (
                        <div className="flex items-center gap-3 p-3 bg-red-50 border border-red-200 rounded-xl text-red-800">
                            ...
                            <div className="text-xs font-bold leading-tight">
                                <span className="uppercase tracking-widest block mb-0.5">Converted to Issue</span>
                                This risk has been closed and converted to a live issue. Manage the resolution in the <span className="underline italic">Issue Registry</span>.
                            </div>
                        </div>
                    )}
                </div>
                ...
                <div className="p-6 overflow-y-auto flex-1 space-y-8">
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="col-span-2">
                                <label className="block text-sm font-medium text-slate-700 mb-1">Risk Title <span className="text-red-500">*</span></label>
                                <input type="text" value={formData.title || ''} onChange={e => handleChange('title', e.target.value)}
                                    ...
                                    placeholder="e.g., Delay in planning approval" required />
                            </div>
                            <div className="col-span-2">
                                <div className="flex items-center justify-between mb-1">
                                    ...
                                    <AIWriter 
                                        type="risk"
                                        context={`Draft a professional risk description for: ${formData.title}. Category: ${formData.category}.`}
                            ...
                            <div className="col-span-2">
                                <label className="block text-sm font-medium text-slate-700 mb-1">Causes & Effects</label>
                                <textarea value={formData.cause || ''} onChange={e => handleChange('cause', e.target.value)}
                                    className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-indigo-500 min-h-[80px]"
                            ...
                            <div>
                                ...
                                <select value={formData.workstream || ''} onChange={e => handleChange('workstream', e.target.value)}
                                    className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-indigo-500">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-1">Linked KRI <span className="text-slate-400 font-normal text-xs">(Optional)</span></label>
                                <select value={formData.kri || ''} onChange={e => handleChange('kri', e.target.value)}
                                    className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-indigo-500">
                                    <option value="">— None —</option>
                                    {KRI_LIST.map(k => <option key={k} value={k}>{k}</option>)}
                                </select>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        <h3 className="text-sm font-semibold text-slate-800 uppercase tracking-wider mb-4 border-b border-slate-100 pb-2">Assessment & Controls</h3>

                        {/* Probability legend */}
                        <div className="mb-4 flex gap-2 flex-wrap">
                            {Object.entries(L_TO_PCT).map(([l, pct]) => (
                                <span key={l} className="px-2 py-0.5 bg-slate-100 text-slate-600 rounded text-[10px] font-bold border border-slate-200">
                                    L{l} = {pct}%
                                </span>
                            ))}
                            <span className="text-[10px] text-slate-400 self-center ml-1">— Likelihood to Probability mapping</span>
                        </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                ...
                                <div>
                                    <h4 className="font-semibold text-slate-800 mb-3 flex items-center justify-between">
                                        Gross Score (Inherent) <span className={clsx("px-2 py-0.5 rounded text-xs border font-bold", scoreColor(currGross))}>{currGross}</span>
                                    </h4>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            <label className="block text-xs font-medium text-slate-600 mb-1">Likelihood (1–5)</label>
                                            <select value={formData.grossL ?? 1} onChange={e => handleChange('grossL', parseInt(e.target.value))}
                                                ...
                                                {[1, 2, 3, 4, 5].map(n => <option key={n} value={n}>{n} — {L_TO_PCT[n]}%</option>)}
                                            </select>
                                        </div>
                                        <div>
                                            <label className="block text-xs font-medium text-slate-600 mb-1">Impact (1–5)</label>
                                            <select value={formData.grossI ?? 1} onChange={e => handleChange('grossI', parseInt(e.target.value))}
                                                className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-indigo-500">
                                    ...
                                    <div className="grid grid-cols-2 gap-4 mt-4">
                                        <div>
                                            <label className="block text-xs font-medium text-slate-600 mb-1">
                                                Probability % <span className="text-indigo-500 font-bold">({formData.grossProb || 0}%)</span>
                                            </label>
                                            <input type="number"
                                            ...
                                            <p className="text-[10px] text-slate-400 mt-0.5">Strictly derived from Likelihood</p>
                                        </div>
                                        <div>
                                            <label className="block text-xs font-medium text-slate-600 mb-1">Exposure / Impact (£)</label>
                                            <input type="number" value={formData.grossImpact ?? 0} onChange={e => handleChange('grossImpact', parseFloat(e.target.value) || 0)}
                                                className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-indigo-500" />
                                        </div>
                                    </div>
                                    <div className="mt-2 text-xs text-slate-600 font-semibold bg-blue-50 rounded-lg px-3 py-2 border border-blue-100">
                                        Gross ALE = £{Math.round(gALE).toLocaleString()} &nbsp;({formData.grossProb || 0}% × £{(formData.grossImpact || 0).toLocaleString()})
                                    </div>
                                </div>
                                ...
                                <div>
                                    <h4 className="font-semibold text-slate-800 mb-3 flex items-center justify-between">
                                        Residual Risk Score <span className={clsx("px-2 py-0.5 rounded text-xs border font-bold", scoreColor(currResidual))}>{currResidual}</span>
                                    </h4>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            <label className="block text-xs font-medium text-slate-600 mb-1">Likelihood (1–5)</label>
                                            <select value={formData.residualL ?? 1} onChange={e => handleChange('residualL', parseInt(e.target.value))}
                                                className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-indigo-500">
                                                {[1, 2, 3, 4, 5].map(n => <option key={n} value={n}>{n} — {L_TO_PCT[n]}%</option>)}
                                            </select>
                                        </div>
                                        <div>
                                            <label className="block text-xs font-medium text-slate-600 mb-1">Impact (1–5)</label>
                                            <select value={formData.residualI ?? 1} onChange={e => handleChange('residualI', parseInt(e.target.value))}
                                                className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-indigo-500">
                                                {[1, 2, 3, 4, 5].map(n => <option key={n} value={n}>{n}</option>)}
                                            </select>
                                        </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-4 mt-4">
                                        <div>
                                            <label className="block text-xs font-medium text-slate-600 mb-1">
                                                Probability % <span className="text-indigo-500 font-bold">({formData.residualProb || 0}%)</span>
                                            </label>
                                            <input type="number"
                                        ...
                                        <div>
                                            <label className="block text-xs font-medium text-slate-600 mb-1">Exposure / Impact (£)</label>
                                            <input type="number" value={formData.residualImpact ?? 0} onChange={e => handleChange('residualImpact', parseFloat(e.target.value) || 0)}
                                                className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-indigo-500" />
                                        </div>
                                    </div>
                                    <div className="mt-2 text-xs text-slate-600 font-semibold bg-indigo-50 rounded-lg px-3 py-2 border border-indigo-100">
                                        Residual ALE = £{Math.round(rALE).toLocaleString()} &nbsp;({formData.residualProb || 0}% × £{(formData.residualImpact || 0).toLocaleString()})
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 gap-4">
                            <div>
                                <div className="flex items-center justify-between mb-1">
                                    <label className="block text-sm font-medium text-slate-700">Current Controls</label>
                                    <AIWriter 
                                        type="risk"
                                        context={`Suggest effective controls for the risk: ${formData.title}. Description: ${formData.desc}.`}
                                        onSuggestion={(val) => handleChange('controls', val)}
                                        className="scale-90"
                                    />
                                </div>
                                <textarea value={formData.controls || ''} onChange={e => handleChange('controls', e.target.value)}
                                    className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-indigo-500 min-h-[80px]"
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select value={formData.response || ''} onChange={e => handleChange('response', e.target.value)}
                                        className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-indigo-500">
                                ...
                                <div>
                                    ...
                                    <select value={formData.appetite || ''} onChange={e => handleChange('appetite', e.target.value)}
                                        className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-indigo-500">
                                        <option value="">— Select Appetite —</option>
                                        {APPETITES.map(a => <option key={a} value={a}>{a}</option>)}
                                    </select>
                                </div>
                            </div>
                        </div>
                    ...
                    <div>
                        <h3 className="text-sm font-semibold text-slate-800 uppercase tracking-wider mb-4 border-b border-slate-100 pb-2">Management</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div className="space-y-2 mb-4">
                                    {riskActions.length === 0 ? (
                                        <div className="text-center py-6 bg-slate-50 rounded-xl border border-dashed border-slate-200">
                                            <p className="text-xs text-slate-400">No actions defined for this risk yet.</p>
                                        </div>
                                    ) : (
                                        riskActions.map(action => (
                                            <div key={action.id} className="group flex items-center justify-between bg-white border border-slate-200 p-3 rounded-xl hover:border-indigo-200 transition-all shadow-sm">
                                                <div className="flex items-start gap-3">
                                                    <button 
                                                        onClick={() => updateTask(action.id, { status: action.status === 'Completed' ? 'Pending' : 'Completed' })}
                                                        className="mt-0.5"
                                                    >
                                                    ...
                                                    <div>
                                                        <p className={clsx("text-sm font-medium", action.status === 'Completed' ? "text-slate-400 line-through" : "text-slate-700")}>
                                                            {action.title}
                                                        </p>
                                                        <div className="flex items-center gap-3 mt-1 text-[10px] text-slate-400">
                                                            <span className="flex items-center gap-1">
                                                                <Calendar className="w-3 h-3" />
                                                                {action.dueDate ? format(new Date(action.dueDate), 'dd MMM yyyy') : 'No date'}
                                                            </span>
                                                            <span className="flex items-center gap-1">
                                    ...
                                    {isAddingAction && (
                                        <div className="bg-white border-2 border-indigo-100 p-4 rounded-xl shadow-md animate-in slide-in-from-top-2 duration-200">
                                            <div className="flex items-center justify-between mb-3">
                                                <span className="text-xs font-bold text-indigo-600 uppercase tracking-wider">New Action</span>
                                                <AIWriter 
                                                    type="risk"
                                                    context={`Suggest a specific, actionable mitigation step for: ${formData.title}.`}
                                            ...
                                            <input 
                                                ...
                                                onKeyDown={async (e) => {
                                                    if (e.key === 'Enter' && newActionTitle && !isAddingTask) {
                                                        try {
                            ...
                            <div>
                                ...
                                <select value={formData.status || 'Open'} onChange={e => handleChange('status', e.target.value)}
                                    className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-indigo-500">
                            ...
                            <div className="flex items-center mt-2">
                                <label className="flex items-center gap-3 cursor-pointer">
                                    <input type="checkbox" checked={!!formData.escalated} onChange={e => handleChange('escalated', e.target.checked)}
                                        className="w-5 h-5 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" />
                                    <span className="text-sm font-medium text-slate-700">Escalate to Programme Level</span>
                                </label>
                            </div>
                        </div>
                    </div>
