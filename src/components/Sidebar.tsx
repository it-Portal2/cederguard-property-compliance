...
import {
  ...
  ClipboardList, PieChart, ScrollText, Gavel, Users, LogOut,
  ChevronDown, FileBarChart, LayoutTemplate, Plus, Target, Wand2,
  Terminal, KeyRound, HelpCircle, Rocket, FileText, Calendar as CalendarIcon, X, Map,
  FolderClosed, MessageSquare, ShieldCheck, Search
} from 'lucide-react';
...
const SidebarFilterContext = React.createContext<{ query: string }>({ query: '' });
...
export function Sidebar() {
  ...
  return (
    <>
      ...
      <div className="border-t border-slate-200 bg-slate-50/60 px-2 py-2 space-y-0.5 pb-safe">
        {/* Avatar row — gradient circle + name + role mono uppercase + cog */}
        <button
          onClick={() => setProfileSettingsOpen(true)}
          className="w-full flex items-center gap-2.5 px-2.5 py-2 rounded-md transition-colors hover:bg-white text-left"
          title="Profile settings"
        >
          <span
            className="inline-flex w-6 h-6 items-center justify-center rounded-full text-[10px] font-semibold text-white shrink-0"
            style={{
              background:
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="block font-mono uppercase tracking-wide text-[10px] text-slate-500 truncate">
              {(user?.role || user?.profile?.role || 'Member').toString().replace(/_/g, ' ')}
            </span>
          </span>
        ...
        <button
          ...
          className="w-full text-left flex items-center gap-2.5 px-2.5 py-2 md:py-1.5 rounded-md text-[13px] font-medium transition-colors text-rose-600 hover:bg-rose-50"
        >
