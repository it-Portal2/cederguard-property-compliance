...
const CheckPill: React.FC<{ label: string; checked: boolean; onChange: () => void }> = ({ label, checked, onChange }) => {
  return (
    <label
      className={clsx(
        "flex items-center gap-3 p-4 rounded-lg border cursor-pointer transition-all select-none text-sm font-semibold",
        checked
...
const inputCls = "w-full border border-slate-200 rounded-lg px-6 py-4 text-sm font-medium focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all bg-white/80 backdrop-blur-sm placeholder:text-slate-400 shadow-sm hover:border-slate-300";
                  ...
                  {qPhase.questions.map((q, qIndex) => {
                    ...
                    return (
                      <div 
                        ...
                        id={`q-container-${q.id}`}
                        className={clsx(
                          "group/q bg-slate-50/30 p-8 rounded-lg border transition-all duration-300",
                      ...
                      >
                        ...
                        <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-8">
                          ...
                          <div className="shrink-0 lg:w-72">
                            {q.type === 'toggle' && (
                              <div className="flex p-1.5 bg-white border border-slate-200 rounded-lg shadow-sm">
                                {['No', 'Yes'].map(option => (
                                  <button
                                    key={option}
