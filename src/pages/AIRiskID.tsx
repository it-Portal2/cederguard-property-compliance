...
import { ScanSearch, AlertTriangle, CheckCircle2, XCircle, ChevronDown, ChevronUp, ShieldAlert, Clock, Info, Eye, RefreshCw, ArrowLeft, Target, AlertCircle, Plus, Check, X, Edit3, Loader2 } from 'lucide-react';
...
export function AIRiskID() {
  const { projectInfo = {}, risks, addRisks, projects, programmes, activeProjectId, activeProgrammeId, setActiveProject, setActiveProgramme, setActiveProgrammeId, suggestedRisks, setSuggestedRisks, updateProject, updateProgramme, strategicRiskAnalysis, setStrategicRiskAnalysis } = useStore();
  ...
  const activeDetails = (activeProjectId ? projects.find(p => p.id === activeProjectId) : programmes.find(p => p.id === activeProgrammeId)) || {} as any;
  ...
  useEffect(() => {
    ...
    const currentEntity = (activeProjectId || pId
      ? projects.find(p => p.id === (activeProjectId || pId))
      : programmes.find(p => p.id === (activeProgrammeId || prId))) as any;
  ...
  const runAnalysis = async () => {
    ...
    const pi = {
      ...
      loc: projectInfo.loc || activeDetails?.location || activeDetails?.geographicScope || activeDetails?.loc,
      scope: projectInfo.scope || activeDetails?.description || activeDetails?.strategicObjectives || activeDetails?.scope
    };
    ...
    if (!pi.name || !pi.type || !pi.scope) {
      ...
      setError(`Validation failed: Please complete the following Setup fields first: ${missing.join(', ')}.`);
      return;
    }

    const restart = searchParams.get('restart') === 'true';

    if (activeDetails.aiRiskDiscoveryDone && !showAnalysisExists && safeSuggestedRisks.length === 0 && !restart) {
    ...
    try {
      ...
      if (!activeProjectId && activeProgrammeId) {
        ...
        const strategicProfile = Array.isArray(strategicResult?.riskProfile) ? strategicResult.riskProfile : [];
        suggestions = strategicProfile.map((s: any) => ({
          ...
          residualL: s.severity === "High" ? 3 : (s.severity === "Medium" ? 2 : 1),
          residualI: s.severity === "High" ? 4 : (s.severity === "Medium" ? 3 : 2),
          response: "Tolerate",
          owner: "Programme Board",
          controls: s.mitigation,
          furtherAction: "Review in Board Meetings",
          grossImpact: s.severity === "High" ? 500000 : (s.severity === "Medium" ? 250000 : 100000),
          grossProb: s.severity === "High" ? 0.8 : (s.severity === "Medium" ? 0.6 : 0.4),
          residualImpact: s.severity === "High" ? 250000 : (s.severity === "Medium" ? 100000 : 50000),
          residualProb: s.severity === "High" ? 0.4 : (s.severity === "Medium" ? 0.3 : 0.2),
          rationale: s.trigger
        }));
      ...
      const currentProgramme = programmes.find(p => p.id === activeProgrammeId);

      setSuggestedRisks(suggestionsArray.map((s: any, i: number) => {
        const getNum = (v: any) => {
        ...
        return {
          ...
          project: currentProject?.name || projectInfo.name || (activeProgrammeId ? 'Shared Portfolio' : 'Project Level'),
  ...
  const safeSuggestedRisks = Array.isArray(suggestedRisks) ? suggestedRisks : [];
  const pendingRisks = safeSuggestedRisks.filter(r => !acceptedIds.includes(r.id) && !rejectedIds.includes(r.id));
  const acceptedRisks = safeSuggestedRisks.filter(r => acceptedIds.includes(r.id));
  const activeName = (activeProjectId ? projects.find(p => p.id === activeProjectId)?.name : programmes.find(p => p.id === activeProgrammeId)?.name) || 'Portfolio';
  ...
  const finalize = async () => {
    try {
        ...
        if (acceptedIds.length === 0 && rejectedIds.length === 0 && safeSuggestedRisks.length > 0) {
          finalAccepted = safeSuggestedRisks.filter(r => !r.exists);
        } else if (finalAccepted.length === 0 && safeSuggestedRisks.filter(r => !r.exists).length > 0) {
          if (!window.confirm("You haven't accepted any risks. Proceed with an empty risk register?")) {
        ...
        if (strategicRiskAnalysis) {
          await api.saveData('strategicRiskAnalysis', strategicRiskAnalysis, pId || prId);
        }

        // Update store with progress and wait for persistence
        if (pId) {
          await updateProject(pId, { riskSetupDone: true, aiRiskDiscoveryDone: true, setupProgress: 75 });
        } else if (prId) {
          await updateProgramme(prId, { riskSetupDone: true, aiRiskDiscoveryDone: true, setupProgress: 75 });
        }
        ...
        setError(`An error occurred while saving: ${err.message || 'Unknown error'}. Please try again.`);
        setIsFinalizing(false);
    }
  };
    ...
    <div className="max-w-[98%] lg:max-w-6xl mx-auto p-2 sm:p-4 lg:p-6 space-y-4 sm:space-y-6 pb-40">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white rounded-3xl shadow-2xl max-w-lg w-full overflow-hidden border border-slate-100 animate-in zoom-in-95 slide-in-from-bottom-8 duration-500">
            <div className="bg-slate-900 p-8 text-white relative">
              ...
              <h3 className="text-xl font-black tracking-tight leading-tight mb-2">Risk Analysis Already Complete</h3>
              <p className="text-slate-400 text-sm leading-relaxed">
                A risk profile has already been established for this entity. 
                Running a new analysis will identify potential threats based on current project data.
              </p>
            </div>
            <div className="p-8 space-y-4">
              <div className="flex flex-col gap-3">
                <button
                  onClick={() => navigate(`/risk/dashboard?from=initiation&type=${activeProjectId ? 'project' : 'programme'}${activeProjectId ? `&projectId=${activeProjectId}` : `&programmeId=${activeProgrammeId}`}`)}
                ...
                <button
                  onClick={() => {
                    ...
                    if (activeProjectId) {
                      ...
                      updateProgramme(activeProgrammeId, { riskSetupDone: false });
                    }
                    setShowAnalysisExists(false);
                    runAnalysis();
                  }}
              ...
              {fromInitiation ? (
                <button
                  onClick={async () => {
                    ...
                    if (contextId) {
                      if (activeProjectId) await updateProject(contextId, { riskSetupDone: true, aiRiskDiscoveryDone: true });
                      else await updateProgramme(contextId, { riskSetupDone: true, aiRiskDiscoveryDone: true });
                    }
          ...
          <div className="flex items-start sm:items-center gap-4 sm:gap-5">
             ...
             <div>
               <h1 className="text-lg sm:text-xl font-black text-white tracking-tight">
                 {activeProjectId ? "Project Risk Analysis complete" : "AI analysis complete"} — {activeName}
               </h1>
               <div className="flex flex-wrap items-center gap-2 sm:gap-3 mt-1.5">
                 <span className="text-[10px] sm:text-xs font-bold text-emerald-400 uppercase tracking-widest">{safeSuggestedRisks.length} risks identified</span>
                 ...
                 <div className="flex flex-wrap items-center gap-2 text-[10px] text-emerald-500/80 font-bold uppercase tracking-wider">
                   ...
                   {projectInfo.riba && <span>• RIBA Stage {projectInfo.riba}</span>}
        ...
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in fade-in slide-in-from-top-4 duration-700">
               ...
               <div className="bg-white rounded-[32px] border border-slate-200 p-8 shadow-xl shadow-slate-200/50 relative overflow-hidden group">
                  ...
                  <div className="flex items-start justify-between mb-6">
                    <div>
                      <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Strategic Summary</h4>
                      <h3 className="text-xl font-black text-slate-900 tracking-tight">Programme Risk Profile</h3>
                    </div>
                    <div className={clsx(
                      "px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border shadow-sm",
                      strategicRiskAnalysis?.summary?.overallRating?.toLowerCase().includes('high') ? "bg-rose-50 text-rose-600 border-rose-100" :
                      strategicRiskAnalysis?.summary?.overallRating?.toLowerCase().includes('medium') ? "bg-orange-50 text-orange-600 border-orange-100" :
                  ...
                  <div>
                    <h5 className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-2">
                      <Target className="w-3 h-3 text-indigo-500" /> Critical Success Factors
                    </h5>
               ...
               <div className="bg-white rounded-[32px] border border-slate-200 p-8 shadow-xl shadow-slate-200/50 relative overflow-hidden group">
                  ...
                  <div className="flex items-start justify-between mb-6">
                    <div>
                      <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Heat Overview</h4>
                      <h3 className="text-xl font-black text-slate-900 tracking-tight">Risk Concentration</h3>
                    </div>
                  ...
                  <div className="space-y-3">
                    <h5 className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2 tracking-widest">Domain Heat Exposure</h5>
                    <div className="grid grid-cols-2 gap-2">
                       {Array.isArray(strategicRiskAnalysis?.heatOverview?.domainHeatMap) && strategicRiskAnalysis.heatOverview.domainHeatMap.map((item: any, i: number) => (
                         <div key={i} className="flex flex-col gap-1.5 p-3 bg-slate-50 rounded-xl border border-slate-100 hover:border-indigo-200 transition-all group/item">
                           <div className="flex items-center justify-between">
                             <span className="text-[10px] font-bold text-slate-500 truncate">{item.domain}</span>
                             <span className={clsx(
                               "text-[10px] font-black",
                               item.score >= 80 ? "text-rose-600" : item.score >= 50 ? "text-orange-600" : "text-emerald-600"
                             )}>{item.score}%</span>
                           </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-4 sm:p-6 shadow-md flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex flex-wrap items-center gap-6 sm:gap-8 justify-center w-full sm:w-auto">
              <div className="flex flex-col items-center sm:items-start shrink-0">
                <span className="text-xl sm:text-2xl font-black text-slate-900">{safeSuggestedRisks.length}</span>
                <span className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest">suggested</span>
              </div>
              ...
              <div className="flex flex-col items-center sm:items-start shrink-0 text-indigo-600">
                <span className="text-xl sm:text-2xl font-black">{safeSuggestedRisks.filter(r => !r.exists).length}</span>
                <span className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest">New Identifications</span>
              </div>
              <div className="flex flex-col items-center sm:items-start shrink-0 text-slate-400">
                <span className="text-xl sm:text-2xl font-black">{safeSuggestedRisks.filter(r => r.exists).length}</span>
                <span className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest">Already Registered</span>
              </div>
              <div className="hidden sm:block w-px h-10 bg-slate-100" />
              <div className="flex flex-col items-center sm:items-start shrink-0">
                <span className="text-xl sm:text-2xl font-black text-emerald-600">{acceptedIds.length}</span>
                <span className="text-[9px] sm:text-[10px] font-black text-emerald-500 uppercase tracking-widest">accepted</span>
              </div>
            </div>
          </div>
          ...
          {safeSuggestedRisks.some(r => r.exists) && (
            <div className="flex items-center gap-2 px-4 py-2 bg-amber-50 border border-amber-100 rounded-xl text-amber-700 text-xs font-medium">
              ...
              <span>Some suggested risks are already in your register for this {activeProjectId ? 'project' : 'programme'}. They are marked and filtered from "Accept All".</span>
            </div>
          )}
        </div>
      )}
      ...
      {safeSuggestedRisks.length === 0 && !loading && (
        <div className="bg-white border border-slate-200 rounded-2xl p-16 text-center shadow-sm">
          ...
          <h2 className="text-2xl font-black text-slate-900 mb-2">Initialise AI Risk Inquiry?</h2>
          <p className="text-slate-500 mb-8 max-w-md mx-auto text-sm leading-relaxed">
            Evaluation of project metadata against standard risk taxonomies to generate a contextual risk profile.
          </p>
          <button onClick={runAnalysis} className="px-8 py-4 bg-indigo-600 text-white font-bold rounded-xl shadow-xl hover:bg-indigo-700 transition-all transform hover:-translate-y-1 active:scale-95">
            Run AI Risk Inquiry
          </button>
        </div>
      )}
      ...
      {loading && (
        <div className="bg-white border border-slate-200 rounded-2xl p-16 flex flex-col items-center justify-center shadow-sm">
          ...
          <h3 className="text-lg font-black text-slate-900">Identifying potential threats...</h3>
          <p className="text-sm text-slate-500 mt-1 animate-pulse">Running semantic analysis on project scope</p>
        </div>
      )}
        ...
        <div className="space-y-4">
          {safeSuggestedRisks.map((r, idx) => {
            ...
            const toggleExpanded = () => {
              if (isExpanded) setExpandedIds(expandedIds.filter(id => id !== r.id));
            ...
            return (
              <div
                ...
                className={clsx(
                  ...
                  isRejected ? "border-rose-200 bg-rose-50/10 opacity-60 grayscale" : 
                  "border-slate-200 hover:border-indigo-300 shadow-sm hover:shadow-lg"
                )}
              >
                {/* Status Indicator Bar */}
                {isAccepted && <div className="absolute top-0 bottom-0 left-0 w-1.5 bg-emerald-500 rounded-l-2xl" />}
                {isRejected && <div className="absolute top-0 bottom-0 left-0 w-1.5 bg-rose-500 rounded-l-2xl" />}
                {!isAccepted && !isRejected && <div className={clsx("absolute top-0 bottom-0 left-0 w-1.5 rounded-l-2xl", r.grossRating >= 16 ? "bg-rose-500" : "bg-orange-500")} />}

                <div className="p-4 sm:p-6">
                  {/* Top Bar: Title & Primary Status */}
                  <div className="flex flex-col sm:flex-row items-start justify-between gap-4 sm:gap-6">
                    <div className="flex items-start gap-3 sm:gap-4 flex-1 min-w-0 w-full">
                      <span className="text-sm font-black text-slate-300 mt-1">{idx + 1}</span>
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-2 mb-2">
                            <h3 className={clsx("text-lg font-black tracking-tight truncate", isRejected ? "text-slate-400 line-through" : "text-slate-900")}>
                          ...
                          {r.exists ? (
                            ...
                            <span className="flex items-center gap-1.5 px-3 py-1 bg-indigo-50 text-indigo-600 text-[10px] font-black uppercase tracking-widest rounded-lg border border-indigo-200/50">
                              <Target className="w-3 h-3 text-indigo-500" /> New Identification
                            </span>
                          )}
                          {isAccepted && (
                        ...
                        <div className="flex flex-wrap items-center gap-2 mb-3">
                          ...
                          <span className={clsx("px-2 py-1 text-[9px] font-black uppercase tracking-widest rounded border shadow-sm", ratingColor)}>
                          ...
                          <span className="px-2 py-1 bg-slate-100 text-slate-500 text-[9px] font-bold uppercase tracking-wider rounded border border-slate-200">
                            {activeProjectId ? activeProjectId : 'Shared'} • {stripMarkdown(r.workstream || 'Contractor')}
                          </span>
                          {r.exists ? (
                            <span className="px-2 py-1 bg-amber-50 text-amber-700 text-[9px] font-black uppercase tracking-widest rounded border border-amber-200 flex items-center gap-1 shadow-sm ring-2 ring-amber-500/20">
                              <AlertTriangle className="w-2.5 h-2.5 text-amber-500" /> Already Registered
                            </span>
                          ) : (
                            <span className="px-2 py-1 bg-emerald-50 text-emerald-700 text-[9px] font-black uppercase tracking-widest rounded border border-emerald-200 flex items-center gap-1 shadow-sm ring-2 ring-emerald-500/20 animate-pulse">
                              <Plus className="w-2.5 h-2.5 text-emerald-500" /> New Identification
                            </span>
                          )}
                          {r.escalated && (
                        ...
                        <p className={clsx("text-sm leading-relaxed max-w-4xl", isRejected ? "text-slate-400" : "text-slate-600")}>
                    ...
                    <div className="flex items-center gap-2 shrink-0 self-end sm:self-auto ml-7 sm:ml-0">
                      {!isAccepted && !isRejected && (
                        <div className="flex items-center gap-1">
                          <button onClick={() => setAcceptedIds([...acceptedIds, r.id])} className="p-2.5 bg-emerald-50 text-emerald-600 hover:bg-emerald-100 rounded-xl transition-all border border-emerald-200/50 shadow-sm flex items-center gap-2 group/btn">
                            <Check className="w-5 h-5" />
                          </button>
                          <button onClick={() => setRejectedIds([...rejectedIds, r.id])} className="p-2.5 bg-rose-50 text-rose-600 hover:bg-rose-100 rounded-xl transition-all border border-rose-200/50 shadow-sm flex items-center gap-2 group/btn">
                            <X className="w-5 h-5" />
                          </button>
                        </div>
                      )}
                      {(isAccepted || isRejected) && (
                        <button 
                          onClick={() => {
                            setAcceptedIds(acceptedIds.filter(id => id !== r.id));
                            setRejectedIds(rejectedIds.filter(id => id !== r.id));
                          }}
                        ...
                        >
                          <X className="w-4 h-4 text-slate-400" /> Reset Selection
                        </button>
                      )}
                      <button 
                    ...
                    <div className="mt-4 sm:mt-6 pt-4 sm:pt-6 border-t border-slate-100 animate-in fade-in slide-in-from-top-2 duration-300">
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
                        <div className="bg-slate-50 border border-slate-100 rounded-xl p-3">
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Likelihood</p>
                          <p className="text-sm font-bold text-slate-800">{r.grossL} — {getProbLabel(r.grossL)}</p>
                        </div>
                        <div className="bg-slate-50 border border-slate-100 rounded-xl p-3">
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Impact</p>
                          <p className="text-sm font-bold text-slate-800">{r.grossI} — {getImpactLabel(r.grossI)}</p>
                        </div>
                        <div className="bg-slate-50 border border-slate-100 rounded-xl p-3">
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Gross Score</p>
                          <p className={clsx("text-sm font-black", ratingColor.split(' ')[0])}>{r.grossRating} — {ratingLabel}</p>
                        </div>
                        <div className="bg-slate-50 border border-slate-100 rounded-xl p-3">
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Risk Response</p>
                          <p className="text-sm font-bold text-slate-800">{stripMarkdown(r.response || 'Reduce')}</p>
                        </div>
                        <div className="bg-slate-50 border border-slate-100 rounded-xl p-3">
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Owner</p>
                          <p className="text-sm font-bold text-slate-800">{stripMarkdown(r.owner || 'Project Manager')}</p>
                        </div>
                        <div className="bg-slate-50 border border-slate-100 rounded-xl p-3">
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Workstream</p>
                          <p className="text-sm font-bold text-slate-800">{stripMarkdown(r.workstream || 'General Safety')}</p>
                        </div>
                        <div className="bg-slate-50 border border-slate-100 rounded-xl p-3">
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Net Score (Est.)</p>
                          <p className="text-sm font-bold text-indigo-600">{r.residualRating} — {getRatingLabel(r.residualRating)}</p>
                        </div>
                        <div className="bg-slate-50 border border-slate-100 rounded-xl p-3">
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Review Freq.</p>
                          <p className="text-sm font-bold text-slate-800">Monthly</p>
                        </div>
                      </div>

                      {isPending && (
                        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3 mt-6 sm:mt-8">
                           <button onClick={() => setAcceptedIds([...acceptedIds, r.id])} className="w-full sm:w-auto justify-center px-4 sm:px-8 py-2.5 sm:py-3 bg-white border-2 border-slate-200 text-slate-900 text-sm font-black rounded-xl hover:bg-slate-50 transition-all flex items-center gap-2.5">
                             <Check className="w-5 h-5 text-emerald-500" /> Accept
                           </button>
                           <button onClick={() => setRejectedIds([...rejectedIds, r.id])} className="w-full sm:w-auto justify-center px-4 sm:px-8 py-2.5 sm:py-3 bg-white border-2 border-slate-200 text-slate-900 text-sm font-black rounded-xl hover:bg-slate-50 transition-all flex items-center gap-2.5">
                             <X className="w-5 h-5 text-rose-500" /> Reject
                           </button>
                           <button onClick={() => { setEditingRisk(r); setIsModalOpen(true); }} className="w-full sm:w-auto justify-center px-4 sm:px-8 py-2.5 sm:py-3 bg-white border-2 border-slate-200 text-slate-900 text-sm font-black rounded-xl hover:bg-slate-50 transition-all flex items-center gap-2.5">
                             <Edit3 className="w-5 h-5 text-indigo-500" /> Edit before accepting
                           </button>
                        </div>
                      )}
                    </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-slate-200 p-3 sm:p-4 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] z-40">
          <div className="max-w-[98%] lg:max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-3">
            <div className="flex items-center justify-between w-full sm:w-auto gap-4 sm:gap-6">
               <button onClick={() => navigate('/risk/register' + (fromInitiation ? '?from=initiation' : ''))} className="shrink-0 flex items-center gap-1 sm:gap-2 px-3 py-2 sm:px-4 hover:bg-slate-100 rounded-xl text-slate-600 text-[10px] sm:text-sm font-bold transition-all">
               ...
               <p className="text-xs sm:text-sm font-bold text-slate-700 flex-1 min-w-0">
                 <span className="text-indigo-600 font-black">{acceptedIds.length}</span> <span className="hidden sm:inline">risks accepted and ready to add to the Register</span> 
                 <span className="sm:hidden">risks ready</span>
                 <span className="text-[9px] sm:text-[10px] text-slate-400 font-medium block truncate">They will be linked to {activeName}</span>
               </p>
            </div>
            ...
            <div className="flex items-center gap-2 sm:gap-4 w-full sm:w-auto">
               <button 
                 onClick={() => {
                   ...
                   setTimeout(() => {
                     const pendingIds = safeSuggestedRisks.filter(r => !r.exists && !acceptedIds.includes(r.id) && !rejectedIds.includes(r.id)).map(r => r.id);
                     setAcceptedIds(prev => [...prev, ...pendingIds]);
                     setIsAcceptingAll(false);
                   }, 150);
                 }}
                 disabled={pendingRisks.filter(r => !r.exists).length === 0 || isAcceptingAll || isFinalizing}
               ...
               >
                 {isAcceptingAll ? <Loader2 className="w-3 h-3 sm:w-4 sm:h-4 text-emerald-500 shrink-0 animate-spin" /> : <Check className="w-3 h-3 sm:w-4 sm:h-4 text-emerald-500 shrink-0" />} <span className="hidden sm:inline">{isAcceptingAll ? "Accepting..." : "Accept all"}</span> remaining ({pendingRisks.filter(r => !r.exists).length})
               </button>
               <button 
                 onClick={finalize}
               ...
               >
                 {isFinalizing ? <Loader2 className="w-3 h-3 sm:w-4 sm:h-4 text-emerald-400 shrink-0 animate-spin" /> : <Plus className="w-3 h-3 sm:w-4 sm:h-4 text-emerald-400 shrink-0" />} {isFinalizing ? "Saving..." : (acceptedIds.length > 0 ? `Add ${acceptedIds.length} risks` : (rejectedIds.length === 0 && safeSuggestedRisks.filter(r => !r.exists).length > 0 ? `Finalise (${safeSuggestedRisks.filter(r => !r.exists).length})` : "Finalise"))}
               </button>
            </div>
          </div>
        </div>
      ...
      <RiskModal
        ...
        onSave={(updated) => {
          setSuggestedRisks(safeSuggestedRisks.map(r => r.id === editingRisk.id ? { ...r, ...updated } : r));
