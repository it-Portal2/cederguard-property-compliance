 ...
 page's primary header signal.*/}
      ...
      <AnimatePresence>
        {pendingItems.length > 0 && (
          ...
          >
            <div className="bg-indigo-600 rounded-lg p-8 mb-8 text-white relative shadow-2xl shadow-indigo-200 overflow-hidden">
              ...
              <div className="mt-8 space-y-3 relative z-10">
                {(showFullQueue ? pendingItems : pendingItems.slice(0, 3)).map((item) => (
                  <motion.div 
                    layout
                    key={item.id}
                    className="bg-white/10 backdrop-blur-md border border-white/20 p-4 rounded-lg flex items-center justify-between gap-4 group hover:bg-white/20 transition-all"
                  >
                ...
                {!showFullQueue && pendingItems.length > 3 && (
                  <button 
                    ...
                    className="w-full py-3 bg-white/5 border border-white/10 rounded-lg text-xs font-bold text-indigo-100 hover:bg-white/10 transition-all"
                  >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        >
          <div className="flex items-center justify-between">
            <div className={`p-2.5 border rounded-lg ${stats.applicable - stats.completed > 0 ? 'bg-indigo-50 border-indigo-100' : 'bg-emerald-50 border-emerald-100'}`}>
      ...
      <div className="bg-gradient-to-br from-indigo-600 to-indigo-800 rounded-lg p-8 text-white shadow-2xl shadow-indigo-200 relative overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-700 delay-300 group">
      ...
      <div className="bg-white p-4 rounded-lg border border-slate-200 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-medium"
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
          />
        </div>
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center bg-slate-50 rounded-lg border border-slate-200 p-1 font-medium text-sm">
            <button
          ...
          <button 
            ...
            className="flex items-center gap-2 px-4 py-2 bg-emerald-50 text-emerald-700 rounded-lg border border-emerald-200 font-medium transition-all shadow-sm hover:bg-emerald-100"
          >
      ...
      <AnimatePresence mode="wait">
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {[...Array(6)].map((_, i) => (
              <div key={i} className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden animate-pulse">
          ...
          <motion.div
            ...
            className="flex flex-col items-center justify-center py-24 px-4 text-center bg-white rounded-lg border-2 border-dashed border-slate-200"
          >
          ...
          >
            {domainStats.map((domain) => (
              <motion.div
                key={domain.id}
                whileHover={{ y: -4 }}
                className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden flex flex-col group border-b-4"
            ...
            {selectedDomainId && (
              <div className="w-full">
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-slate-100 text-slate-600 rounded-lg font-bold text-xs hover:bg-slate-200 transition-all active:scale-95 w-fit"
                >
                  ...
                  {DOMAINS.map(domain => {
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          "w-full flex items-center justify-between px-4 py-3 rounded-lg text-sm font-bold transition-all group",
                          isActive 
                            ? "bg-indigo-600 text-white shadow-lg shadow-indigo-100" 
                            : "text-slate-600 hover:bg-slate-50"
                        )}
                                          ...
                                          <div className="mt-4 pt-4 border-t border-slate-100">
                                            <div className="relative group">
                                               <textarea 
                                                ...
                                                className="w-full bg-white border border-slate-200 rounded-lg p-3 text-[11px] font-medium focus:ring-2 focus:ring-indigo-500/20 transition-all min-h-[80px]"
                                                onKeyDown={async (e) => {
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <select 
                    ...
                    className="w-full bg-slate-50 border-none rounded-lg px-4 py-3 text-sm font-bold focus:ring-2 focus:ring-indigo-500/20"
                  >
