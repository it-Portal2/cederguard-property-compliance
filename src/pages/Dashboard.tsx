...
export function Dashboard() {
  ...
  const contextRisks = safeRisks.filter(r => {
    ...
    if (activeProgrammeId) {
      const isProgLevel = r.programmeId === activeProgrammeId || !!r.isProgrammeLevel;
  ...
  const contextIssues = safeIssues.filter(i => {
    ...
    if (activeProgrammeId) {
      const isProgLevel = i.programmeId === activeProgrammeId || !!i.isProgrammeLevel;
  ...
  const issueOpen = contextIssues.filter(i => i.status !== "4. Resolved").length;
  const issueEscalated = contextIssues.filter(i => i.status === "2. Escalated").length;
  ...
  const handleGenerateInsights = async () => {
    ...
    try {
      const insights = await analyzeStrategicInsights({
        compliance: { total: compTotal, complete: compComplete, pct: compPct, highRisk: compHighRisk },
        risks: { total: riskTotal, open: riskOpen, high: riskHigh, ale: riskResidualALE },
        issues: { total: issueTotal, open: issueOpen, escalated: issueEscalated },
      ...
      setAiError(err instanceof ApiError ? err : (err.message || 'Failed to generate AI insights. Please try again.'));
    } finally {
  ...
  return (
    <div className="space-y-6 max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-bold text-slate-900">
              {activeProjectId ? 'Active Project' : (activeProgrammeId ? 'Active Programme' : 'Portfolio Aggregate')}
            </h3>
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">
              {activeProject?.name || activeProgramme?.name || 'All Authorized Projects'}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 bg-slate-50 p-1.5 rounded-xl border border-slate-200/60 w-full md:w-auto">
          <div className="flex items-center gap-2 px-3 py-1.5 bg-white border border-slate-200 rounded-lg shadow-sm w-full md:w-auto">
            <span className="text-[10px] font-black text-slate-500 uppercase tracking-wider shrink-0">Context:</span>
            <select
            ...
            >
              <option value="all">{isProjectManager ? "Portfolio Aggregate (All)" : "My Projects (Aggregate)"}</option>
              
              {isProjectManager && displayProgrammes.length > 0 && (
                <optgroup label="BY PROGRAMME">
                  {displayProgrammes.map(p => (
                    <option key={p.id} value={`programme:${p.id}`} title={stripMarkdown(p.name)}>{stripMarkdown(p.name)}</option>
                  ))}
                </optgroup>
              )}
              ...
              {displayProjects.length > 0 && (
                <optgroup label="BY PROJECT">
                  {displayProjects.map(p => (
                    <option key={p.id} value={`project:${p.id}`} title={stripMarkdown(p.name)}>{stripMarkdown(p.name)}</option>
                  ))}
                </optgroup>
              )}
            </select>
      ...
      {isViewingAsPM && isClientAdmin && activeProjectId && (() => {
        ...
        return (
          <div className="flex items-center justify-between gap-4 bg-amber-50 border border-amber-200 rounded-2xl px-5 py-3 shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs font-black text-amber-800 uppercase tracking-widest">Admin Backdoor — PM View</p>
                <p className="text-xs text-amber-700 font-medium mt-0.5">
                  You are viewing <strong>{proj?.name || 'this project'}</strong> as the Project Manager. This is a read-only administrative oversight view.
                </p>
              </div>
            </div>
            <button
              onClick={() => { setActiveProject(null); navigate('/projects'); }}
        ...
        >
          ...
          <div className="p-4 md:p-8">
            ...
            <div className={clsx("grid gap-4 mb-8", isClientAdmin ? "grid-cols-2 lg:grid-cols-5" : "grid-cols-2 lg:grid-cols-4", loadingOverview && "hidden")}>
              ...
              <div className="bg-slate-50 rounded-2xl p-5 border border-slate-100 transition-all hover:bg-white hover:shadow-lg hover:shadow-indigo-500/5 group">
                <div className="text-3xl font-black text-slate-900 group-hover:text-indigo-600 transition-colors">{displayProjects.length}</div>
                <div className="text-[10px] text-slate-400 mt-1 font-black uppercase tracking-widest">Total Projects</div>
              </div>
              <div className="bg-slate-50 rounded-2xl p-5 border border-slate-100 transition-all hover:bg-white hover:shadow-lg hover:shadow-emerald-500/5 group">
                <div className="text-3xl font-black text-emerald-600">{displayProjects.filter((p: any) => p.isPublished).length}</div>
                <div className="text-[10px] text-slate-400 mt-1 font-black uppercase tracking-widest">Published</div>
              </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-1 space-y-3">
                <p className="text-[11px] text-slate-400 font-black uppercase tracking-widest ml-1">Strategic Monitoring</p>
                <div className="space-y-2">
                  <Link to="/monitoring/kri" className="flex items-center justify-between p-4 bg-white border border-slate-100 rounded-2xl hover:border-indigo-200 hover:shadow-md transition-all group">
                    <div className="flex items-center gap-3">
                      <div className="p-2 bg-indigo-50 rounded-lg group-hover:bg-indigo-100">
                        <TrendingUp className="w-4 h-4 text-indigo-600" />
                      </div>
                      <span className="text-sm font-bold text-slate-700">KRI Tracker</span>
                    </div>
                    <ChevronRight className="w-4 h-4 text-slate-300 group-hover:text-indigo-400" />
                  </Link>
                  <Link to="/monitoring/alerts" className="flex items-center justify-between p-4 bg-white border border-slate-100 rounded-2xl hover:border-amber-200 hover:shadow-md transition-all group">
                    <div className="flex items-center gap-3">
                      <div className="p-2 bg-amber-50 rounded-lg group-hover:bg-amber-100">
                        <ShieldAlert className="w-4 h-4 text-amber-600" />
                      </div>
                      <span className="text-sm font-bold text-slate-700">Alerts & Thresholds</span>
                    </div>
              ...
              <div className="lg:col-span-2 space-y-3">
                <div className="flex items-center justify-between ml-1">
                  <p className="text-[11px] text-slate-400 font-black uppercase tracking-widest">Recent Activity</p>
                  <Link to="/projects" className="text-[10px] font-black text-indigo-600 hover:text-indigo-700 uppercase tracking-widest">View All</Link>
                </div>
                {displayProjects.length > 0 ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                    {displayProjects.slice(0, 4).map((p: any) => (
                      <div 
                        key={p.id} 
                        onClick={() => { setActiveProject(p.id); setActiveProgramme(null); document.querySelector('main')?.scrollTo(0,0); if (isClientAdmin && !isViewingAsPM) { navigate('/dashboard?viewAs=pm'); } }}
                  ...
                  <div className="bg-slate-50 rounded-2xl p-8 text-center border border-dashed border-slate-200">
                    <p className="text-xs text-slate-400 font-medium">No active project records found.</p>
                  </div>
                )}
              </div>
            </div>
      ...
      {!activeProjectId && !activeProgrammeId && !isClientAdmin && safeProjects.length === 0 && (
        <div className="flex flex-col items-center justify-center py-10 text-center">
          ...
          <p className="text-sm text-slate-500 max-w-sm mb-6">
            You haven't created any projects yet. Use the Project Initiation wizard to set up your first project.
          </p>
          <button
            onClick={() => { setActiveProject(null); setActiveProgramme(null); navigate('/initiate'); }}
      ...
      {(activeProjectId || activeProgrammeId || (!isClientAdmin && safeProjects.length > 0) || (isClientAdmin && !activeProjectId && !activeProgrammeId)) && (
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 md:gap-6">
          <div className="flex-1 min-w-0">
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
              <h2 className="text-xl font-bold text-slate-900 truncate">
                {activeProjectId ? 'Project Dashboard' : isProjectManager ? 'Portfolio Dashboard' : 'Projects Overview'}
              </h2>
              {(!activeProjectId) && (
                <span className="shrink-0 w-max bg-indigo-600 text-white text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-widest shadow-sm">
                  {activeProgrammeId ? 'Programme View' : isProjectManager ? 'Aggregate Portfolio' : 'All Projects'}
                </span>
              )}
            </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <button
              ...
              className={`px-4 py-2 text-xs sm:text-sm font-medium rounded-xl transition-all disabled:opacity-50 whitespace-nowrap border ${confirmClear
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 relative z-10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-sm font-bold text-white tracking-tight">Compliance Verification Required</h3>
                  <p className="text-[10px] text-indigo-100 font-bold uppercase tracking-widest">{pendingComplianceCount} AI-generated requirements are waiting for your approval.</p>
                </div>
              </div>
              <Link 
          ...
          <div className="bg-slate-900 rounded-2xl border border-slate-800 overflow-hidden shadow-xl relative">
            ...
            <div className="bg-white/5 border-b border-white/10 px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative z-10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-sm font-bold text-white">{isProjectManager && !activeProjectId ? 'Portfolio' : 'Project'} Strategic Intelligence</h3>
                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">AI executive portfolio analysis</p>
                </div>
              </div>
              {!strategicInsights ? (
                <button
                ...
                >
                  {generatingInsights ? <Loader2 className="w-3 h-3 animate-spin" /> : <ScanSearch className="w-3 h-3 text-white/80" />}
                  {generatingInsights ? 'Analyzing...' : 'Generate AI Insight'}
                </button>
              ) : (
              ...
              {!strategicInsights && !aiError ? (
                <div className="flex flex-col items-center justify-center py-10 text-center bg-white/5 rounded-xl border border-dashed border-white/10">
                  ...
                  <p className="text-sm text-slate-400 font-medium max-w-xs leading-relaxed">
                    Analyze cross-functional compliance, risk, and issue data to generate senior-level strategic guidance.
                  </p>
                </div>
              ) : strategicInsights && !aiError ? (
                      ...
                      <div className="relative mb-6">
                        <svg className="w-24 h-24 transform -rotate-90">
                          ...
                          <circle
                            ...
                            strokeDashoffset={263.8 - (263.8 * (strategicInsights.healthScore || 0)) / 100}
                            className={clsx(
                              'transition-all duration-1000 ease-out',
                              strategicInsights.healthScore > 70 ? 'text-emerald-400' :
                                strategicInsights.healthScore > 40 ? 'text-amber-400' : 'text-red-400'
                            )}
                          />
                        </svg>
                        <div className="absolute inset-0 flex flex-col items-center justify-center">
                          <span className="text-2xl font-black text-white">{strategicInsights.healthScore || 0}%</span>
                          <span className="text-[8px] font-bold text-slate-500 uppercase tracking-widest">Health</span>
                        </div>
                      </div>
                      <h4 className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2">Portfolio Health</h4>
                      <p className="text-[10px] text-slate-400 leading-relaxed max-w-[120px]">{stripMarkdown(strategicInsights.healthRationale)}</p>
                    </div>
                  </div>
                </div>
              ) : null}
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {displayProjects.length > 0 ? (
                  ...
                  >
                    <div className="flex justify-between items-start mb-3">
                      ...
                      <span className={clsx(
                        ...
                        project.status === 'Draft' ? 'bg-amber-50 text-amber-700 border border-amber-100' :
                        'bg-slate-50 text-slate-700 border border-slate-100'
                      )}>
                        {project.status || 'Draft'}
                      </span>
                    </div>
                    <h3 className="text-sm font-bold text-slate-800 leading-tight group-hover:text-indigo-600 transition-colors truncate" title={stripMarkdown(project.name)}>
                    ...
                    <div className="mt-3 flex items-center justify-between text-[10px] text-slate-500 font-bold uppercase tracking-wider">
                      ...
                      <div className="flex items-center gap-1">
                        <span className={clsx(
                          ...
                          project.rag === 'Red' ? 'bg-red-500' : project.rag === 'Amber' ? 'bg-amber-500' : 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.4)]'
                        )} />
                        {project.rag || 'Green'}
                      </div>
                    </div>
                ...
                <div className="col-span-full py-8 text-center bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                  <p className="text-sm text-slate-400 font-medium">No projects found. Use Project Initiation to create one.</p>
                </div>
              )}
            </div>
            ...
            {displayProjects.length > 8 && (
              <div className="flex justify-center mt-2">
                <Link to="/projects" className="text-xs font-bold text-slate-500 hover:text-indigo-600 transition-colors flex items-center gap-1.5 opacity-70">
            ...
            <div className="bg-indigo-900 rounded-2xl p-4 border border-indigo-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-0 shadow-lg relative overflow-hidden group">
              ...
              <div className="flex items-center gap-4 relative z-10">
                ...
                <div>
                  <h3 className="text-sm font-bold text-white">Regulations active for construction programme</h3>
                  <p className="text-[10px] text-indigo-300 font-medium leading-normal max-w-xs">
                    Access building safety and property compliance regulations relevant to your active projects.
                  </p>
                </div>
              </div>
              <Link 
        ...
        {(activeProject?.milestones?.length || activeProgramme?.milestones?.length) ? (
          <div className="space-y-4 mt-6">
              ...
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-4 text-[10px] font-black uppercase tracking-widest text-slate-400">
                  <div className="flex items-center gap-1.5">
                    <div className="w-2 h-2 rounded-full bg-amber-400" /> Key Milestone
                  </div>
                  <div className="flex items-center gap-1.5">
                    <div className="w-2 h-2 rounded-full bg-indigo-400" /> Upcoming
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
              <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    <h3 className="text-xs font-black text-slate-900 uppercase tracking-widest">Critical Path & RIBA Milestones</h3>
                    <span className="px-1.5 py-0.5 bg-indigo-100 text-indigo-700 text-[9px] font-black rounded uppercase">Next 6 Events</span>
                  </div>
                  <Link 
                    to={activeProject ? '/initiate' : '/projects/programme-setup'} 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-slate-100">
                { (() => {
                  const allMilestones = (activeProject?.milestones || activeProgramme?.milestones || []);
                  const keyMilestones = allMilestones.filter(m => m.isKey).sort((a,b) => new Date(a.date).getTime() - new Date(b.date).getTime());
                  const otherMilestones = allMilestones.filter(m => !m.isKey).sort((a,b) => new Date(a.date).getTime() - new Date(b.date).getTime());
                  
                  // Show up to 6 total: Priority to Key, then others
                  const displayMilestones = [...keyMilestones, ...otherMilestones].slice(0, 6);

                  if (displayMilestones.length === 0) return (
                    <div className="col-span-2 p-12 text-center text-slate-400 italic text-sm font-medium">No upcoming milestones defined.</div>
                  );

                  return displayMilestones.map(m => (
                    <div key={m.id} className="p-5 flex items-center justify-between gap-4 hover:bg-slate-50/80 transition-all group relative">
                      {m.isKey && (
                      ...
                      <div className="flex items-center gap-4 min-w-0">
                        <div className={clsx(
                          ...
                          m.isKey ? "bg-amber-50 border-amber-100 text-amber-700" : "bg-indigo-50 border-indigo-100 text-indigo-700",
                          m.status === 'Completed' && "bg-emerald-50 border-emerald-100 text-emerald-700"
                        )}>
                          <span className="text-[10px] font-black uppercase leading-none">{m.stage || 'S?'}</span>
                          {m.isKey ? <Star className="w-2.5 h-2.5 mt-0.5 fill-amber-500 text-amber-500" /> : <Calendar className="w-2.5 h-2.5 mt-0.5" />}
                        </div>
                        <div className="min-w-0">
                          <div className="flex items-center gap-2">
                            <h4 className="text-sm font-bold text-slate-900 truncate pr-2" title={stripMarkdown(m.name)}>{stripMarkdown(m.name)}</h4>
                            {m.isKey && <Star className="w-3 h-3 text-amber-500 fill-amber-500 shrink-0" />}
                          </div>
                          <div className="flex items-center gap-2 mt-0.5 overflow-hidden">
                            ...
                            {m.description && (
                              <>
                                ...
                                <span className="text-[11px] text-slate-500 truncate">{stripMarkdown(m.description)}</span>
                              </>
                            )}
                          </div>
                        </div>
                      ...
                      <div className="text-right shrink-0">
                          <div className={clsx(
                            ...
                            m.status === 'Delayed' ? 'text-red-600' : 'text-slate-900'
                          )}>{new Date(m.date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}</div>
                          <div className={clsx(
        ...
        {!activeProjectId && !activeProgrammeId && isProjectManager && (
          <div className="mt-8">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-lg font-bold text-slate-800">Portfolio Risk Aggregation</h2>
            </div>
            <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
              <div className="xl:col-span-3 bg-white rounded-3xl border border-indigo-100 shadow-sm overflow-hidden relative group">
                ...
                <div className="px-6 py-5 border-b border-slate-100 bg-slate-50/30 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-xs font-black text-slate-900 uppercase tracking-widest">Top 5 Portfolio-Wide Risks</h3>
                      <p className="text-[10px] text-slate-500 font-bold">Aggregated across all active projects and programmes</p>
                    </div>
                  </div>
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead className="bg-slate-50/50 text-slate-400 text-[10px] font-black uppercase tracking-widest border-b border-slate-100">
                      <tr>
                        ...
                        <th className="px-6 py-4 text-center">Inherent Score</th>
                        <th className="px-6 py-4 text-right">ALE Exposure</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-50">
                      {topRisks.length > 0 ? topRisks.map((risk) => {
                        const score = risk.grossRating || 0;
                        const scoreColor = score >= 16 ? 'text-red-600 bg-red-50 border-red-100' : score >= 12 ? 'text-amber-600 bg-amber-50 border-amber-100' : 'text-emerald-600 bg-emerald-50 border-emerald-100';
                        const sourceProject = safeProjects.find(p => p.id === risk.projectId);
                        const sourceProgramme = safeProgrammes.find(p => p.id === risk.programmeId);

                        return (
                          <tr key={risk.id} className="hover:bg-slate-50/80 transition-all group">
                            <td className="px-6 py-4">
                              <div className="flex items-center gap-3">
                                <AlertTriangle className={clsx("w-4 h-4 shrink-0", score >= 16 ? "text-red-500" : "text-amber-500")} />
                                <div className="min-w-0">
                                  <div className="font-bold text-slate-900 truncate max-w-[250px]" title={stripMarkdown(risk.title)}>{stripMarkdown(risk.title)}</div>
                                  <div className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter">{risk.category}</div>
                                </div>
                              </div>
                            </td>
                            <td className="px-6 py-4">
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-xs font-bold text-slate-600 truncate max-w-[150px]">
                                  {sourceProject?.name || sourceProgramme?.name || 'Global Context'}
                                </span>
                              </div>
                            </td>
                            <td className="px-6 py-4 text-center">
                              <span className={clsx("inline-flex items-center justify-center w-8 h-8 rounded-xl font-black text-sm border shadow-sm", scoreColor)}>
                                {score}
                              </span>
                            </td>
                            <td className="px-6 py-4 text-right">
                              <div className="text-sm font-mono font-black text-slate-900">£{(risk.residualALE || 0).toLocaleString()}</div>
                              <div className="text-[9px] text-slate-400 uppercase tracking-widest font-bold">Residual Liability</div>
                            </td>
                          </tr>
                        );
                      }) : (
                        <tr>
                          <td colSpan={4} className="px-6 py-16 text-center">
                            <div className="flex flex-col items-center gap-2 opacity-40">
                              ...
                              <p className="text-sm font-medium text-slate-500 italic">No critical risks identified across portfolio.</p>
                            </div>
                          </td>
                        </tr>
                      )}
              ...
              <div className="flex flex-col gap-4">
                <div className="bg-linear-to-br from-indigo-900 to-slate-900 rounded-3xl p-6 border border-indigo-800 shadow-xl relative overflow-hidden group">
                  ...
                  <div className="relative z-10">
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <h3 className="text-xs font-black text-white uppercase tracking-widest">KRI Pulse</h3>
                    </div>
                    <div className="space-y-4">
                      <div>
                        <div className="flex justify-between items-end mb-1">
                          <span className="text-[10px] font-bold text-indigo-300 uppercase">Exposure Index</span>
                          <span className="text-lg font-black text-white">{riskTotal > 0 ? Math.round((riskHigh / riskTotal) * 100) : 0}%</span>
                        </div>
                        <div className="h-1.5 bg-indigo-950 rounded-full overflow-hidden border border-indigo-800/50">
                          <div 
                            ...
                            style={{ width: `${riskTotal > 0 ? Math.round((riskHigh / riskTotal) * 100) : 0}%` }} 
                          />
                        </div>
                      </div>
                      <div className="pt-2">
                        <h4 className="text-[10px] font-bold text-indigo-400 uppercase tracking-wider mb-2">Portfolio Sentiment</h4>
                ...
                <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-sm flex-1 flex flex-col justify-between">
                  <div>
                    <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Aggregation Logic</h3>
                    <p className="text-[11px] text-slate-600 font-medium leading-relaxed">
                      Risks are normalized across project tiers using gross rating and financial exposure. AI identifies cross-programme dependencies that may compound systemic liability.
                    </p>
                  </div>
        ...
        <div className="space-y-4">
          ...
          {isLoading ? (
            ...
            <>
              ...
              <div className="bg-white rounded-2xl border border-slate-200 p-4 md:p-5 flex flex-col md:flex-row md:items-center gap-4 md:gap-6 shadow-sm">
                ...
                <div className="flex-1 h-3 bg-slate-100 rounded-full overflow-hidden shadow-inner">
                  <div className="h-full bg-linear-to-r from-emerald-500 to-teal-400 transition-all duration-1000 ease-out" style={{ width: `${compPct}%` }} />
                </div>
                <div className="text-xl font-black text-emerald-600 truncate">{compPct}%</div>
              </div>
            </>
          ) : (
            <div className="bg-white rounded-2xl border-2 border-dashed border-slate-200 p-8 flex flex-col items-center justify-center text-center">
              ...
              <h3 className="text-sm font-bold text-slate-800">Compliance Setup Pending</h3>
          ...
          {isLoading ? (
            ...
            <>
              <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
                ...
                <StatCard label="Open Risks" value={riskOpen} color="red" border="border-l-red-500" />
                <StatCard label="High/Severe Risks" value={riskHigh} color="amber" border="border-l-amber-500" info="Risks with a gross rating of 16 or higher (Major or Severe)." />
                <StatCard label="Escalated Risks" value={riskEscalated} color="purple" border="border-l-purple-500" info="Risks that have been escalated to the Programme Board for senior attention." />
                <StatCard label="Residual ALE" value={`£${Math.round(riskResidualALE / 1000)}k`} color="green" border="border-l-emerald-500" info="Residual Annual Loss Expectancy: The estimated financial exposure after all controls are applied." />
              </div>
                ...
                <div className={clsx("bg-white rounded-2xl border border-slate-200 p-6 shadow-sm hover:shadow-md transition-shadow", (!activeProjectId && !activeProgrammeId) && "lg:col-span-3")}>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-lg font-extrabold text-red-600">{issueEscalated}</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-white rounded-2xl border-2 border-dashed border-slate-200 p-8 flex flex-col items-center justify-center text-center">
              ...
              <h3 className="text-sm font-bold text-slate-800">Risk Identification Pending</h3>
              <p className="text-xs text-slate-500 mt-1 max-w-sm">
...
function StatCard({ label, value, color, border, info }: { label: string, value: string | number, color: string, border: string, info?: string }) {
  ...
  return (
    <div className={clsx('bg-white rounded-xl border border-slate-200 border-l-4 px-4 py-4 shadow-sm hover:shadow-md transition-all', border)}>
      <div className={clsx('text-xl font-black mb-1 truncate', colors[color])}>{value}</div>
