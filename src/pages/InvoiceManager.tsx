...
function SectionCard({
  ...
  return (
    <div className={clsx('bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden transition-all', isExcluded && 'opacity-60 bg-slate-50 grayscale-[0.5]')}>
...
function PricingBadge({ label, recommended }: { label: string; recommended?: boolean }) {
  return (
    <span className={clsx('text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wide', recommended ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500')}>
      {recommended ? '★ ' : ''}{label}
    </span>
  );
}
  ...
  return (
    <div id="invoice-capture" className="bg-white p-12 w-[800px] text-slate-900 border border-slate-100" style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>
      ...
      <div className="flex justify-between items-start mb-12">
        <div>
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h1 className="text-xl font-black tracking-tight text-slate-900 leading-none">CEDAR GUARD</h1>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Risk Intelligence & Compliance</p>
            </div>
          </div>
        </div>
        <div className="text-right">
          <h2 className="text-2xl font-black text-slate-200 mb-2">INVOICE</h2>
      ...
      <table className="w-full mb-12">
        <thead>
          <tr className="border-b-2 border-slate-100 text-[10px] font-bold text-slate-400 uppercase tracking-widest text-left">
      ...
      <div className="flex justify-end">
        <div className="w-64 space-y-3">
          ...
          <div className="flex justify-between text-lg font-black bg-slate-900 text-white p-4 rounded-xl mt-4">
            <span>TOTAL DUE</span>
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[10px] font-bold px-2 py-0.5 bg-orange-100 text-orange-600 rounded-full uppercase tracking-wider">Super Admin</span>
          </div>
      ...
      {activeTab === 'calculator' ? (
        <>
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm flex items-center gap-4">
            <div className="flex items-center gap-2 text-xs font-bold text-slate-400 uppercase tracking-widest bg-slate-50 px-3 py-2 rounded-lg border border-slate-100 shrink-0">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-[380px_1fr] gap-6">
            ...
            <div className="space-y-5">
              ...
              <div className="rounded-2xl p-6 text-white shadow-xl bg-gradient-to-br from-indigo-900 to-indigo-800">
                <div className="flex items-center justify-between mb-2">
                  <p className="text-xs font-bold uppercase tracking-widest text-white/50">Monthly Platform Cost</p>
                  <span className="px-2 py-0.5 bg-white/10 rounded-full text-[10px]">Infrastructure Only</span>
                </div>
                <div className="flex items-end gap-3">
                  <p className="text-5xl font-black tabular-nums">{fmtInt(totalCostMonthlyGBP)}</p>
                ...
                <div className="mt-6 pt-6 border-t border-white/10 grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-2xl font-black text-emerald-400">{fmtInt(totalCostMonthlyGBP * marginMultiplier)}</p>
                  </div>
                  <div>
                    <p className="text-xs text-white/50 mb-1 uppercase font-bold tracking-tight">Monthly Profit</p>
                    <p className="text-2xl font-black text-indigo-300">+{fmtInt((totalCostMonthlyGBP * marginMultiplier) - totalCostMonthlyGBP)}</p>
                  </div>
                </div>
              </div>
          ...
          <div className="divide-y divide-slate-100">
            {invoices.length === 0 ? (
              <div className="p-12 text-center">
                <div className="p-3 bg-indigo-600/10 rounded-2xl border border-indigo-100/50 flex items-center justify-center mx-auto mb-4 w-16 h-16">
              ...
              invoices.map((inv: any) => (
                <div key={inv.id} className="p-6 hover:bg-slate-50 transition-colors flex items-center justify-between group">
                  <div className="flex items-center gap-4">
                    <div className="p-3 bg-white border border-slate-200 rounded-xl shadow-sm text-indigo-600 group-hover:bg-indigo-50 group-hover:border-indigo-100 transition-all">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 mt-1">
                        ...
                        <span className="text-[10px] font-black px-1.5 py-0.5 bg-indigo-50 text-indigo-600 rounded uppercase tracking-wider">
                          {inv.margin}% Margin
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="flex items-center gap-6">
                    <div className="text-right">
                      <p className="text-xl font-black text-slate-900 font-mono">£{Number(inv.total).toLocaleString()}</p>
                      <p className="text-[10px] font-bold text-emerald-600 mt-0.5 uppercase tracking-widest">Profit: £{Number(inv.profit).toLocaleString()}</p>
                    </div>
      ...
      {showInvoicePreview && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-2xl max-h-[95vh] overflow-y-auto relative animate-in zoom-in-95 duration-300">
            <button 
