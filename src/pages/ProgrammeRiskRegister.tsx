...
export function ProgrammeRiskRegister() {
    ...
    const [deEscalateModal, setDeEscalateModal] = useState<{ risk: any } | null>(null);
    ...
    return (
                                ...
                                <tr key={r.id} className={clsx('hover:bg-slate-50/80 transition-all group border-b border-slate-100 items-center', isEsc ? 'bg-orange-50/30' : 'bg-purple-50/10', selectedIds.includes(r.id) && 'bg-indigo-50/40')}>
                                    ...
                                    <td className="px-3 py-3 sticky right-0 bg-white group-hover:bg-slate-50 z-20 border-l border-slate-100 transition-colors shadow-[-4px_0_10px_-3px_rgba(0,0,0,0.1)]">
                                         <div className="flex items-center gap-1.5">
                                             ...
                                             {canModify && isEsc && (
                                                 <button onClick={() => setDeEscalateModal({ risk: r })}
        ...
        {/* De-escalation Confirmation Modal — outside motion.div to avoid transform containing-block issue */}
        {deEscalateModal && (
            <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="text-base font-bold text-slate-800">De-escalate Risk</h3>
                            <p className="text-xs text-slate-500 mt-0.5">This action will return the risk to project level</p>
                        </div>
                    </div>
                    <div className="bg-slate-50 rounded-xl p-3 border border-slate-100">
                        <p className="text-xs font-bold text-slate-700">{deEscalateModal.risk.title}</p>
                        <p className="text-[11px] text-slate-500 mt-1">
                            This risk will be removed from the Programme Risk Register and returned to the originating project register with status reset to <span className="font-bold text-slate-700">Open</span>.
                        </p>
                    </div>
                    <div className="flex items-center gap-2 justify-end pt-1">
                        <button
                            onClick={() => setDeEscalateModal(null)}
                        ...
                        <button
                            onClick={async () => {
                                ...
                                try {
                                    await updateRisk(deEscalateModal.risk.id, { escalated: false });
                                    toast.success('Risk de-escalated and returned to project level.');
                                    setDeEscalateModal(null);
                                } catch {
                                    toast.error('Failed to de-escalate risk. Please try again.');
                                } finally {
                                    setDeEscalating(false);
                                }
                            }}
                            disabled={deEscalating}
                            className="px-4 py-2 text-sm font-bold text-white bg-amber-600 hover:bg-amber-700 rounded-xl transition-colors disabled:opacity-50 flex items-center gap-2"
                        >
                            {deEscalating ? (
                                <><span className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin inline-block" />De-escalating…</>
                            ) : 'Confirm De-escalate'}
                        </button>
                    </div>
                </div>
