...
import {
    Building2, Search, Plus, Trash2, Edit2, Download, Loader2,
    Filter, X, ChevronDown, ChevronRight, BookOpen, Layers, Archive, RefreshCw
} from 'lucide-react';
...
export function Projects() {
    const {
        projects, activeProjectId, programmes, activeProgrammeId,
        setActiveProject, setActiveProgramme,
        deleteProject, archiveProject, unarchiveProject, loadProjectData, user, addNotification
    } = useStore();
    ...
    const schemeTypes = Array.from(new Set((Array.isArray(projects) ? projects : []).map(p => p.type || p.schemeType).filter(Boolean)));
    const ribaStages = [
    ...
    const filtered = (Array.isArray(projects) ? projects : []).filter(p => {
        ...
        const matchProg = programmeFilter === 'All' || p.programmeId === programmeFilter;
        const matchRag = ragFilter === 'All' || (p.rag || 'Green') === ragFilter;
        const matchRiba = !ribaFilter || (p.riba || '').includes(ribaFilter.split('-')[0].trim());
        const matchScheme = schemeFilter === 'All' || (p.type || p.schemeType) === schemeFilter;
        ...
        const isVisible = isSuperAdmin(user?.email, userRole) || 
                         (userRole === 'client_admin') ||
                         (p.pmId === user?.uid || p.projectManagerId === user?.uid || p.createdBy === user?.uid || p.createdBy === user?.email) ||
                         (user?.profile?.clientId && p.clientId === user.profile.clientId);

        return matchSearch && matchProg && matchRag && matchRiba && matchScheme && matchFlag && matchArchived && isVisible;
    });

    // Stats
    const total = filtered.length;
    const redCount = filtered.filter(p => (p.rag || 'Green') === 'Red').length;
    const amberCount = filtered.filter(p => (p.rag || 'Green') === 'Amber').length;
    const greenCount = filtered.filter(p => (p.rag || 'Green') === 'Green').length;

    const isAdminView = isAtLeastClientAdmin(userRole);

    const handleOpen = async (id: string, route: string = '/dashboard?viewAs=pm') => {
        setOpeningId(id);
        try {
    ...
    const visibleProjects = (Array.isArray(projects) ? projects : []).filter(p => {
        return isSuperAdmin(user?.email, userRole) || 
               (userRole === 'client_admin') ||
               (p.pmId === user?.uid || p.projectManagerId === user?.uid || p.createdBy === user?.uid || p.createdBy === user?.email) ||
               (user?.profile?.clientId && p.clientId === user.profile.clientId);
    });

    const ragCounts: Record<string, number> = { All: visibleProjects.length, Red: 0, Amber: 0, Green: 0 };
    visibleProjects.forEach(p => { ragCounts[p.rag || 'Green'] = (ragCounts[p.rag || 'Green'] || 0) + 1; });

    const progCounts: Record<string, number> = { All: visibleProjects.length };
    visibleProjects.forEach(p => { if (p.programmeId) progCounts[p.programmeId] = (progCounts[p.programmeId] || 0) + 1; });
    
    const handleDeleteProject = async (e: React.MouseEvent, id: string, name: string) => {
        e.stopPropagation();
        if (window.confirm(`Are you sure you want to delete project "${name}"? This will delete all associated data.`)) {
            try {
                await api.deleteProject(id);
                deleteProject(id);
            } catch (err) {
                console.error('Failed to delete project:', err);
                addNotification({ title: 'Delete Failed', body: 'Failed to delete project. Please try again.', type: 'system' });
            }
        }
    };
    ...
    const handleArchiveProject = async (e: React.MouseEvent, id: string, name: string, isArchived: boolean) => {
        e.stopPropagation();
        ...
        if (window.confirm(`Are you sure you want to ${action} project "${name}"?`)) {
            try {
                if (isArchived) {
                    ...
                    addNotification({ title: 'Project Unarchived', body: `"${name}" has been restored.`, type: 'system' });
                } else {
                    await archiveProject(id);
                    addNotification({ title: 'Project Archived', body: `"${name}" has been moved to archives.`, type: 'system' });
                }
            } catch (err) {
                console.error(`Failed to ${action} project:`, err);
                addNotification({ title: 'Action Failed', body: `Failed to ${action} project.`, type: 'system' });
            }
        }
    };
            ...
            <div className="flex flex-col md:flex-row gap-8">
                ...
                <div className="md:hidden sticky top-[3.5rem] z-30 bg-slate-50/80 backdrop-blur-md -mx-4 px-4 py-3 border-b border-slate-200 space-y-3 mb-6">
                    <div className="flex items-center justify-between">
                        <div>
                            <h1 className="text-lg font-black text-slate-900 tracking-tight leading-none mb-1">My projects</h1>
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            >
                                ...
                                Filters
                                {(programmeFilter !== 'All' || ragFilter !== 'All' || ribaFilter || schemeFilter !== 'All' || flagFilter) && (
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <div className="hidden md:flex flex-row items-end justify-between gap-4 mb-8">
                        <div>
                            <div className="flex items-center gap-3 mb-1">
                                <h1 className="text-xl font-black text-slate-900 tracking-tight">My projects</h1>
                            ...
                            <div className="flex flex-wrap items-center gap-2 text-sm text-slate-500 font-medium">
                                <span className="text-indigo-600 font-bold">
                                    {isAdminView 
                                        ? (programmeFilter === 'All' ? 'All Programmes' : (Array.isArray(programmes) ? programmes : []).find(p => p.id === programmeFilter)?.name)
                                        : 'All Projects'
                                    }
                                </span>
                                <span>·</span>
                                <span>{isAdminView ? 'Admin view' : 'Project Manager view'}</span>
                                {!isAdminView && (
                                    <>
                                        <span>·</span>
                                        <span className="text-emerald-600 font-bold">Created By You</span>
                                    </>
                                )}
                            </div>
                            <p className="text-xs text-slate-400 mt-1">Select any project to view details</p>
                        </div>
                        <div className="flex items-center gap-3">
                            ...
                            {canCreate && (
                                <button
                                    onClick={() => { setActiveProject(null); setActiveProgramme(null); navigate('/projects/new'); }}
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
                        {[
                            ...
                            { label: 'RAG amber', value: amberCount, color: 'text-amber-500' },
                            { label: 'RAG green', value: greenCount, color: 'text-emerald-600' },
                        ].map(({ label, value, color }) => (
                            <div key={label} className="bg-white border border-slate-200 rounded-xl p-4 text-center md:text-left">
                                <p className={clsx('text-xl font-black truncate', color)}>{value}</p>
                                <p className="text-[10px] md:text-xs text-slate-500 mt-0.5 font-bold uppercase tracking-wider">{label}</p>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="space-y-3">
                        {filtered.length === 0 ? (
                            <div className="bg-white border border-slate-200 rounded-2xl p-16 flex flex-col items-center text-center">
                                ...
                                <p className="text-sm text-slate-400 mt-1 max-w-xs">Adjust your filters or create a new project to get started.</p>
                                <button
                            ...
                            filtered.map(project => {
                                ...
                                const ref = project.reference || ('P' + (project.id || '').substring(0, 3).toUpperCase());
                                ...
                                return (
                                    ...
                                    >
                                        <div className="relative">
                                            ...
                                            <div className={clsx('absolute left-0 top-0 bottom-0 w-1 md:hidden', ragColor(rag))} />
                                            
                                            <div className="px-5 py-5 md:py-4 flex items-center gap-4">
                                                {/* Ref badge */}
                                                <span className="shrink-0 px-2.5 py-1 bg-slate-100 text-slate-600 rounded-lg text-[10px] md:text-[11px] font-black tracking-wide hidden sm:block">{ref}</span>
                                                ...
                                                >
                                                    <div className="flex items-center gap-2 min-w-0 mb-1">
                                                        <span className="font-bold text-slate-900 text-base leading-tight truncate" title={project.name || 'Untitled Project'}>{project.name || 'Untitled Project'}</span>
                                                        {project.isHRB && (
                                                            <span className="px-1.5 py-0.5 bg-orange-100 text-orange-700 rounded text-[9px] font-black uppercase tracking-wider shrink-0">HRB</span>
                                                        )}
                                                        <div className={clsx('w-2 h-2 rounded-full shrink-0 hidden md:block', ragColor(rag))} />
                                                        <span className={clsx('text-xs font-bold shrink-0 hidden md:block', ragTextColor(rag))}>{rag}</span>
                                                        {project.isArchived && (
                                                            <span className="px-1.5 py-0.5 bg-slate-100 text-slate-500 rounded text-[9px] font-black uppercase tracking-wider shrink-0 border border-slate-200">Archived</span>
                                                        )}
                                                        {openingId === project.id && <Loader2 className="w-3.5 h-3.5 text-indigo-400 animate-spin shrink-0" />}
                                                    </div>
                                                    <p className="text-xs text-slate-500 truncate font-medium" title={[project.type || project.schemeType, project.units ? `${project.units} units` : null, project.riba ? `Stage ${project.riba.split(' ')[0]}` : null, project.updatedAt ? `Updated ${timeAgo(project.updatedAt)}` : null].filter(Boolean).join(' · ')}>
                                                        {[
                                                            project.type || project.schemeType, 
                                                            project.units ? `${project.units} units` : null, 
                                                            project.riba ? `Stage ${project.riba.split(' ')[0]}` : null,
                                                            project.updatedAt ? `Updated ${timeAgo(project.updatedAt)}` : null
                                                        ].filter(Boolean).join(' · ')}
                                                    </p>
                                                </button>
                                                ...
                                                <div className="flex items-center gap-1 shrink-0">
                                                    ...
                                                    {canEdit && (
                                                        <button
                                                            onClick={(e) => handleArchiveProject(e, project.id, project.name || 'Untitled', !!project.isArchived)}
                                                            className={clsx(
                                                        ...
                                                        >
                                                            {project.isArchived ? <RefreshCw className="w-4 h-4 md:w-3.5 md:h-3.5" /> : <Archive className="w-4 h-4 md:w-3.5 md:h-3.5" />}
                                                        </button>
                                                    )}
        ...
        return (
            <div className="space-y-6">
                    ...
                    <FilterSection title="Programme">
                        <div className="space-y-1">
                            ...
                            >
                                ...
                                <span className={clsx('text-[10px] px-2 py-0.5 rounded-full font-bold', programmeFilter === 'All' ? 'bg-indigo-50 text-white' : 'bg-slate-200 text-slate-600')}>{(Array.isArray(programmes) ? programmes : []).length}</span>
                            </button>
                            {(Array.isArray(programmes) ? programmes : []).map(prog => (
                                ...
                                >
                                    <span className="truncate text-left" title={prog.name}>{prog.name}</span>
                                    <span className={clsx('text-[10px] px-1.5 py-0.5 rounded-full font-bold shrink-0 ml-1', programmeFilter === prog.id ? 'bg-indigo-100 text-indigo-600' : 'bg-slate-100 text-slate-400')}>{progCounts[prog.id] || 0}</span>
                                </button>
                            ))}
                        </div>
                    </FilterSection>
                ...
                <FilterSection title="Quick Access">
                    <div className="space-y-1">
                        <div className="mt-2 space-y-0.5 max-h-[200px] overflow-y-auto pr-1 custom-scrollbar">
                            ...
                            {filtered.length > 10 && (
                                <p className="text-[10px] text-slate-400 px-3 py-1 italic">+{filtered.length - 10} more...</p>
                            )}
                        </div>
                    </div>
                </FilterSection>
                ...
                <FilterSection title="RAG Status">
                    <div className="grid grid-cols-2 gap-1.5">
                        {(['All', 'Red', 'Amber', 'Green'] as const).map(r => (
                            ...
                            >
                                {r !== 'All' && <div className={clsx('w-2 h-2 rounded-full', ragColor(r))} />}
                                <span>{r}</span>
                            </button>
                        ))}
                    </div>
                        ...
                        <div className="space-y-0.5">
                            <button
                                ...
                                className={clsx(
                                    ...
                                    !ribaFilter ? 'bg-white text-indigo-700 font-bold shadow-sm border border-indigo-100' : 'text-slate-500 hover:bg-indigo-50/30 hover:text-indigo-600 hover:shadow-sm'
                                )}
                            >
                                All Stages
                            </button>
                            {ribaStages.map(s => {
                                ...
                                const count = filtered.filter(p => (p.riba || '').includes(stageCode)).length;
                                return (
                                    <button
                                        key={s}
                                        ...
                                        className={clsx(
                                            ...
                                            ribaFilter === s ? 'bg-white text-indigo-700 font-bold shadow-sm border border-indigo-100' : 'text-slate-500 hover:bg-indigo-50/30 hover:text-indigo-600 hover:shadow-sm'
                                        )}
                                    >
                                        <span className="truncate text-left" title={s}>{s}</span>
                                        <span className="text-[9px] opacity-60 font-black ml-2">{count}</span>
                                    </button>
                                );
                            })}
                        </div>
                ...
                <FilterSection title="Scheme Types">
                    <div className="space-y-1">
                         <button
                            ...
                            className={clsx(
                                'w-full text-left px-3 py-1.5 text-xs rounded-lg transition-all duration-300',
                                schemeFilter === 'All' ? 'bg-white text-indigo-700 font-bold shadow-sm border border-indigo-100' : 'text-slate-500 hover:bg-indigo-50/30 hover:text-indigo-600 hover:shadow-sm'
                            )}
                        >
                            All Types
                        </button>
                        {schemeTypes.map(st => {
                            const c = (Array.isArray(projects) ? projects : []).filter(p => (p.type || (p as any).schemeType) === st).length;
                            return (
                                <button
                                    key={st}
                                    ...
                                    className={clsx(
                                        ...
                                        schemeFilter === st ? 'bg-white text-indigo-700 font-bold shadow-sm border border-indigo-100' : 'text-slate-500 hover:bg-indigo-50/30 hover:text-indigo-600 hover:shadow-sm'
                                    )}
                                >
                ...
                <FilterSection title="Flags">
                    <div className="space-y-1">
                        {[
                            { key: 'HRB', label: 'HRB schemes', count: (Array.isArray(projects) ? projects : []).filter(p => p.isHRB).length },
                            { key: 'Overdue', label: 'Overdue actions', count: (Array.isArray(projects) ? projects : []).filter(p => (p.overdueCount || 0) > 0).length },
                            { key: 'Leaseholders', label: 'Leaseholders', count: (Array.isArray(projects) ? projects : []).filter(p => p.hasLeaseholders).length },
                        ].map(({ key, label, count }) => (
                            <button
                                key={key}
                                ...
                                className={clsx(
                                    ...
                                    flagFilter === key ? 'bg-amber-50 text-amber-700 border border-amber-200' : 'text-slate-500 hover:bg-slate-50 border border-transparent'
                                )}
                            >
                                <span>{label}</span>
                                <span className={clsx('text-[9px] px-1.5 py-0.5 rounded-full font-black', flagFilter === key ? 'bg-amber-200 text-amber-800' : 'bg-slate-100 text-slate-400')}>{count}</span>
                            </button>
                        ))}
                    </div>
                </FilterSection>
