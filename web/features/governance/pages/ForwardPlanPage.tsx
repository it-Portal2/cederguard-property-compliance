...
} from '../components/forwardPlan/types';
import {
  ForwardPlanItemModal,
  type FpBodyOption,
} from '../components/forwardPlan/ForwardPlanItemModal';
import { ForwardPlanCalendarView } from '../components/forwardPlan/ForwardPlanCalendarView';
import { ForwardPlanTimelineView } from '../components/forwardPlan/ForwardPlanTimelineView';
import { ForwardPlanWorkflowView } from '../components/forwardPlan/ForwardPlanWorkflowView';
import { ForwardPlanImportModal } from '../components/forwardPlan/ForwardPlanImportModal';
import { SchedulePlannerView } from '../components/forwardPlan/SchedulePlannerView';
...
// Default board-column layout. Each entry pairs a display label with a
// framework body's seed `id` so we can resolve the loaded body's `_id`
// (= the `boardGates` lookup key) at render time.
const DEFAULT_BOARD_COLUMNS: Array<{ label: string; bodyId: string }> = [
...
export function GovernanceForwardPlanPage() {
  ...
  // "how do they identify which one it is"). Sorted oldest-
  ...
  const proposedItems = useMemo(() => {
    ...
    //  banner is suppressed in historical mode (no actions to
  ...
  // PgM confirms a Proposed FP item (auto-publishes + syncs
  ...
  // full column-for-column mirror of the real Forward
  ...
  // resolving each canonical slot to the loaded framework body's
  ...
  const columns = useMemo<ColumnDef<ForwardPlanItem>[]>(() => {
    ...
    const boardCols: ColumnDef<ForwardPlanItem>[] = DEFAULT_BOARD_COLUMNS.map(
      ({ label, bodyId }) => ({
  ...
  const filters: FilterDef<ForwardPlanItem>[] = useMemo(
    () => [
      ...
      // Excel Column F filter. "Not set" matches both null
 ...
 past month. Sits above StatsCards so the user can't miss it.*/}
      ...
      {/*re-routing banner. Server flags FP items + reports
 ...
 re-routing is the more urgent signal.*/}
      ...
      {/*rose banner above the body when Proposed items exist.
 Lists the pending items inline so PgM can click straight through to
 confirm/decline (client feedback: "how do they identify
 ...
 `historicalView.loading` flips true while a past
